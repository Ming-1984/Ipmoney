# Engineering 文档

## 状态
- 项目状态：docs/engineering/project-status.md
- 全面质量整改 TODO（主执行清单，2026-03-05）：docs/engineering/full-quality-todo-2026-03-05.md
- 测试报告（最新结果）：docs/engineering/test-report.md
- 历史规划归档（2026-02~2026-03）：docs/engineering/archive/legacy-roadmaps-2026-02-to-2026-03.md
- 页面-接口-测试矩阵（2026-03-05）：docs/engineering/page-api-test-matrix-2026-03-05.md
- 漏洞台账（2026-03-05）：docs/engineering/vulnerability-ledger-2026-03-05.md

## 发布与运维
- 生产过渡：docs/engineering/production-transition.md
- 发布检查清单：docs/engineering/release-checklist.md
- 系统测试计划：docs/engineering/system-test-plan.md
- 环境变量：docs/engineering/environments.md
- DB 预检：docs/engineering/db-preflight-check.md
- DB 备份/恢复：docs/engineering/db-backup-restore.md
- 权限矩阵：docs/engineering/permissions-matrix.md

## API 与契约
- OpenAPI 契约：docs/api/openapi.yaml
- OpenAPI 覆盖：docs/engineering/openapi-coverage.md
- 未使用接口处置：docs/engineering/openapi-unused-endpoints-decision.md
- 可追溯矩阵：docs/engineering/traceability-matrix.md
- Mock 说明：docs/engineering/mocking.md

## UI 与前端
- UI 指南：docs/engineering/ui-guidelines.md
- WeApp 包体根因报告（2026-03-05）：docs/engineering/weapp-bundle-root-cause-2026-03-05.md
- Token 映射：docs/engineering/token-mapping.md
- 硬编码白名单：docs/engineering/hardcode-whitelist.md
- 前端骨架说明：docs/engineering/frontend-skeleton.md

## 工程基础
- 技术选型：docs/engineering/tech-stack.md
- 仓库结构：docs/engineering/repo-structure.md
- 术语表：docs/engineering/terminology.md

## 架构
- 架构索引：docs/architecture/README.md

## 归档
- 历史规划归档（2026-02~2026-03）：docs/engineering/archive/legacy-roadmaps-2026-02-to-2026-03.md
