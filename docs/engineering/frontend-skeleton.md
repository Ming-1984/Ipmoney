...
补充（P0 必含）：
- Search 分类新增「书画专区」，具备筛选/排序/详情/咨询/收藏。
- 书画专区筛选收敛为：类别 / 创作年份（起止年份选择） / 价格类型 / 价格区间（三档 + 面议）。
- 新增书画专区详情页、发布页、我的书画管理页（状态/审核态）。
- 书画详情页信息区展示著作权登记证书编号（如有）。
- 书画专区结果卡片左右布局：仅展示作者/年份与价格信息，隐藏标签与浏览/收藏统计。
- 新增技术经理人栏目（列表/详情/咨询入口），入口来自首页快捷入口。
- 专利发布表单包含：可交付清单/预计周期/可谈空间/质押与许可现状声明。

## 5. 后台骨架范围（P0 必演示）

- 认证审核（企业/科研院校/政府/协会/技术经理人）：通过/驳回（带原因）
- 上架审核：通过/驳回、重复/冒用权属提示与处置入口
- 书画审核：通过/驳回、证书编号/权属材料核验入口
...
- 技术经理人栏目管理：展示开关/推荐权重/服务标签维护（与认证状态联动）
...
- **用户端：** Home/Search 智能体入口（语义检索）已接入；H5 与小程序均可用。
- **详情页：** 专利/需求/成果/书画详情已接入 AI 解析卡片 + 评分/纠错入口（基础版）。
- **后台：** CMS 已落地基础版（挂牌/需求/成果/书画）；数据地图扩展仍为 P1；AI 解析复核、告警中心、专利托管已落地基础版。
