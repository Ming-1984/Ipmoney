...
- **0 禁用词**：用户可见区域不得出现 `P0/演示/Mock/undefined/HTTP xxx/堆栈`（可用 `node scripts/scan-banned-words.mjs` 扫描）
...
- 字号一致：同屏不出现“部分偏小/偏大”；用户可见文本最小字号≥`24rpx≈12px`；H5 禁用自动字体调整（`text-size-adjust: 100%`）；H5 root clamp `18–22`；NutUI 与页面单位一致；禁止手写 `html/:root font-size`
...
- 滚动：页面仅有一个主滚动容器；遮罩不允许滚动穿透；列表/聊天的 ScrollView 有明确高度
- 弹层滚动：内容超出视口时内部可滚动；背景锁定；关闭后滚动恢复；安全区不遮挡
...
- WeApp 顶部栏：不出现“双顶部栏/双返回按钮”；若启用 `navigationStyle: custom`，需验证 safe-area 与胶囊按钮区域不遮挡
