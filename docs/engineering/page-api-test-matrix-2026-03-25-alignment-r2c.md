...
- Coverage split: client 45/45, admin 21/21.
- UI HTTP smoke: 22/66 pages have route-level HTTP checks (root-focused).
- DOM assertion automation: 66/66 pages (66/66 in current DOM smoke run).
- E2E automation: 0/66 (planned in B03).
...
| admin-home-announcements | admin | /home-announcements | admin/config-home-announcements | Y | Y | Y | N | pending | admin-web |
...
| client-patent-map | client | /subpackages/patent-map/index | public discovery/patent-map | N | Y | Y | N | pending | client |
...
- DOM smoke is running in `full-batch2` and currently covers 66/66 pages; full page coverage is achieved for this baseline.
