...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r220`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r220/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`409/409`)
  - Coverage in this batch: existing domain strictness suites remained green, and new controller-level coverage landed for `test/my-verification.controller.spec.ts` (`3`) + `test/industry-tags.controller.spec.ts` (`3`) + `test/organizations.controller.spec.ts` (`2`) + `test/inventors.controller.spec.ts` (`1`) + `test/notifications.controller.spec.ts` (`2`) + `test/regions.controller.spec.ts` (`2`) across user verification delegation, admin industry-tag permission boundary, and public org/inventor/notification/region delegation paths.
  - Notes: this batch hardens verification/tags/org/inventor/notification/region controller boundaries without touching real login/payment integrations.
