...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r198`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r198/`
...
- `pnpm -C apps/api test`
  - Result: pass (`167/167`)
  - Coverage in this batch: `test/conversations.filters.spec.ts` (`4`), `test/patents.filters.spec.ts` (`4`), `test/alerts.filters.spec.ts` (`4`), `test/cases.filters.spec.ts` (`4`), `test/notifications.filters.spec.ts` (`4`), `test/user-verifications.filters.spec.ts` (`2`), `test/reports.filters.spec.ts` (`4`), `test/contracts.filters.spec.ts` (`4`), `test/announcements.filters.spec.ts` (`4`), `test/ai.query-filters.spec.ts` (`4`), `test/patent-map.filters.spec.ts` (`5`), `test/tech-managers.filters.spec.ts` (`4`), `test/content-utils.sanitize.spec.ts` (`5`), `test/regions.filters.spec.ts` (`6`), `test/org-inventor.filters.spec.ts` (`6`), `test/listings.search-filters.spec.ts` (`6`), `test/audit-logs.read.spec.ts` (`5`), `test/orders.write.spec.ts` (`24`), `test/comments.write.spec.ts` (`28`), `test/favorites.write.spec.ts` (`29`), `test/addresses.write.spec.ts` (`9`), `test/health.e2e-spec.ts` (`2`).
  - Notes: `regionCode` filter strictness is now aligned to 6-digit validation in `inventors/organizations/tech-managers`; patent-map dry-run import regression now covers duplicate row and missing-region error convergence; AI query filter sanitization now includes hidden industry-tag suppression and strict admin filter pagination; announcement public list now has service-level hidden-tag sanitization regression guards; contract list now has strict status/pagination filter regression coverage; reports now has strict `days/start/end` range-filter regression guards for summary/export paths; case/notification/user-verification/alerts/patents/conversations list paths now have dedicated strict filter regression suites for query-shape convergence.
