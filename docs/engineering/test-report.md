...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r218`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r218/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`383/383`)
  - Coverage in this batch: existing domain strictness suites remained green, and new controller-level coverage landed for `test/rbac.controller.spec.ts` (`6`) + `test/favorites.controller.spec.ts` (`5`) across role-management delegation and multi-entry favorites delegation paths.
  - Notes: this batch hardens RBAC/favorites controller boundaries without touching real login/payment integrations.
