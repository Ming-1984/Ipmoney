...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r236`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r236/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`524/524`)
  - Coverage in this batch: existing domain strictness suites remained green, and listings write-flow coverage expanded with `test/listings.write-flow.spec.ts` (`7`) on user/admin create-update, submit/off-shelf, and publish/off-shelf/approve/reject strict branches.
  - Notes: this batch strengthens listing write-path validation, ownership/file checks, status transitions, and notification/audit branches without touching real login/payment integrations.
