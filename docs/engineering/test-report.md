...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r231`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r231/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`489/489`)
  - Coverage in this batch: existing domain strictness suites remained green, and case write-flow coverage expanded with `test/cases.write-flow.spec.ts` (`7`) on create/assign/status/note/evidence/sla branches.
  - Notes: this batch strengthens case write-path/state-transition and relation-validation branches without touching real login/payment integrations.
