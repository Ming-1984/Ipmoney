...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r230`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r230/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`482/482`)
  - Coverage in this batch: existing domain strictness suites remained green, and announcements write/status lifecycle coverage expanded with `test/announcements.write-status.spec.ts` (`7`) on create/update/publish/off-shelf/delete/detail-hidden branches.
  - Notes: this batch strengthens announcements write-path/state-transition validation branches without touching real login/payment integrations.
