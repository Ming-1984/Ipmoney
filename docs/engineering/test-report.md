...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ApiBaseUrl https://staging-api.example.com -ApiPort 3200 -ReportDate 2026-03-06`
  ...
  - API smoke: pass (474/474) -> `.tmp/api-real-smoke-2026-03-06-summary.json`
  - API smoke write/read split: writes 382/382, reads 92/92.
...
- AI/region/featured/file-temporary-access probes (demo auth/payment)
  ...
  - Added admin write coverage: `PUT /admin/regions/:regionCode/industry-tags` (deterministic via created tag seed) and `PUT /admin/listings/:id/featured` (CITY/NONE success paths) with persistence assertions.
  - Added admin negative coverage: region-industry-tags invalid body (400), missing `industryTags` field (400), invalid region code (400), missing region (404); listing-featured missing region for CITY (400), invalid featuredLevel (400), invalid featuredRank (400), invalid featuredUntil (400), missing listing (404).
...
### Risks still open
- API write-path assertions are now 382 checks (with 92 read-back semantic verifications), and unique write-operation coverage is at least the previous 132/135 baseline plus report-import and AI/industry-tag/region/featured/file-access additions; remaining risk is mainly deeper transaction-isolation windows under broader multi-actor parallel writes.
