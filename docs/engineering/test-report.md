...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r225`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r225/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`450/450`)
  - Coverage in this batch: existing domain strictness suites remained green, and service behavior coverage expanded with `test/alerts.acknowledge.spec.ts` (`4`) + `test/organizations.detail.spec.ts` (`3`).
  - Notes: this batch strengthens admin alert acknowledge and organization detail behavior branches without touching real login/payment integrations.
