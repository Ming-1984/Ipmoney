...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r207`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r207/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`274/274`)
  - Coverage in this batch: existing domain strictness suites remained green, and `test/webhooks.service.spec.ts` was expanded from `7` to `12` with final-payment derivation, pending-payment update path, paid-status idempotent no-op, refund fallback, and unresolved-payType safe-ignore branches.
  - Notes: this batch deepens webhook payment/refund transition rigor (without touching real payment integration) while preserving prior addresses/demo-auth/webhook/config/RBAC/favorites/orders/patent-maintenance/files anti-pollution and strict-filter checks.
