...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r217`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r217/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`372/372`)
  - Coverage in this batch: existing domain strictness suites remained green, and new controller-level coverage landed for `test/audit-logs.controller.spec.ts` (`3`) + `test/alerts.controller.spec.ts` (`3`) + `test/reports.controller.spec.ts` (`2`) across admin permission boundary and delegation-fallback paths.
  - Notes: this batch hardens admin alerts/audit/reports controller boundaries without touching real login/payment integrations.
