...
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1 -ReportDate 2026-03-13-r232`
  - Result: success (all steps)
  - Render artifact: `docs/demo/rendered/ui-smoke-2026-03-13-r232/` (core mode, 3 pages)
...
- `pnpm -C apps/api test`
  - Result: pass (`496/496`)
  - Coverage in this batch: existing domain strictness suites remained green, and patent-maintenance write-flow coverage expanded with `test/patent-maintenance.write-flow.spec.ts` (`7`) on schedule/task create-update strict branches.
  - Notes: this batch strengthens maintenance schedule/task write-path validation, relation checks, and conflict/audit branches without touching real login/payment integrations.
