...
- `pnpm -C apps/api test`
  ...
  - Notes: this batch further added RBAC read/update strictness coverage (auth boundary, fallback-role mapping, strict `userId/roleIds` validation and unknown-role rejection), while preserving prior favorites/orders/patent-maintenance anti-pollution and strict-filter checks.
