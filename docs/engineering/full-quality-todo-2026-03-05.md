...
- `smoke`: pass (API 495/495, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- [x] B04 Upgrade `api-real-smoke` from read-heavy to read-write balanced.
  - Acceptance: write coverage raised from 1.48% to >=20% (phase 1). (done: >=132/135 unique operations covered, currently 403 write assertions executed in smoke)
...
| B04 | done | Codex | 2026-03-06 | 2026-03-06 | `api-real-smoke` expanded to 495/495 (writes 403/403 + reads 92/92 semantic assertions), with unique write-operation coverage already near-full baseline |
