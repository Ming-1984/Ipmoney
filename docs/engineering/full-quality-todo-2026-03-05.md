...
- `smoke`: pass (API 232/232, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 232 checks (including semantic read-back/state assertions).
- Write operations total 135; smoke now executes 170 write assertions plus 62 read-back semantic checks (state transitions, persistence, cross-module link integrity, same-key replay invariants, and selected concurrency race outcomes), while unique write-operation coverage remains at least prior 132/135 baseline.
...
| B04 | done | Codex | 2026-03-06 | 2026-03-06 | `api-real-smoke` expanded to 232/232 (writes 170/170 + reads 62/62 semantic assertions), with unique write-operation coverage already near-full baseline |
| B02 | in_progress | Codex | 2026-03-06 | - | write batch now includes same-key replay invariants + semantic read-back checks (order/refund/case/maintenance/rbac/report/import) + duplicate-maintenance-schedule 409 regression guard + concurrency race matrices (maintenance create, refund approve/reject, order payout, transfer-vs-refund overlap); remaining depth is deeper transaction-isolation windows |
...
- Deliverables:
  ...
  20) next step: extend transaction-isolation windows with higher parallel fan-out (3+ concurrent writes) across refund/invoice/settlement domains (pending).
