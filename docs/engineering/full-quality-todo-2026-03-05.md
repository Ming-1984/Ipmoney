...
- `smoke`: pass (API 244/244, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 244 checks (including semantic read-back/state assertions).
- Write operations total 135; smoke now executes 180 write assertions plus 64 read-back semantic checks (state transitions, persistence, cross-module link integrity, same-key replay invariants, and selected concurrency race outcomes), while unique write-operation coverage remains at least prior 132/135 baseline.
...
| B04 | done | Codex | 2026-03-06 | 2026-03-06 | `api-real-smoke` expanded to 244/244 (writes 180/180 + reads 64/64 semantic assertions), with unique write-operation coverage already near-full baseline |
| B02 | in_progress | Codex | 2026-03-06 | - | write batch now includes same-key replay invariants + semantic read-back checks (order/refund/case/maintenance/rbac/report/import) + duplicate-maintenance-schedule 409 regression guard + concurrency race matrices (maintenance create, refund approve/reject, order payout pair/triple, transfer-vs-refund overlap); remaining depth is deeper transaction-isolation windows |
...
- Deliverables:
  ...
  21) next step: extend multi-endpoint 3+ fan-out races across refund/invoice/settlement mixed writes under the same order aggregate (pending).
