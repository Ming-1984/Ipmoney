...
- `smoke`: pass (API 816/816, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 816 checks (including semantic read-back/state assertions).
...
| B04 | done | Codex | 2026-03-06 | 2026-03-07 | `api-real-smoke` expanded to 816/816 (writes 565/565 + reads 251/251 including 93 semantic read-back assertions), with unique write-operation coverage already near-full baseline |
...
- Deliverables:
  ...
  39) RBAC privilege matrix deepening + permission deny-path normalization landed (done: custom role `report.read` now has explicit allow assertion on finance summary, forbidden assertions on high-risk admin read/write endpoints, unauthenticated 401 boundary checks are now explicit for key protected endpoints, and `requirePermission` now returns consistent 403 instead of internal 500 on deny path).
