...
- `smoke`: pass (API 377/377, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 377 checks (including semantic read-back/state assertions).
- Write operations total 135; smoke now executes 292 write assertions plus 85 read-back semantic checks (state transitions, persistence, cross-module link integrity, same-key replay invariants, and selected concurrency race outcomes), while unique write-operation coverage remains at least prior 132/135 baseline.
...
- Deliverables:
  ...
  28) next step: add larger-seed chaos sampling + percentile-based stability thresholding to make probabilistic regressions measurable across repeated runs (pending).
