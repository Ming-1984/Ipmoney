...
- `smoke`: pass (API 799/799, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 799 checks (including semantic read-back/state assertions).
- Write operations total 135; smoke now executes 561 write assertions plus 93 read-back semantic checks (state transitions, persistence, cross-module link integrity, same-key replay invariants, selected concurrency race outcomes, percentile-based chaos stability guard, cross-run trend-threshold capability, and AI/industry-tag/region/featured/file-temporary-access/admin-listings-write/user-listings-write/admin-cases-create-enum/patent-strict-negative/patent-maintenance-create-status/ai-query-strict-enum/order-payment-intent-strict-pay-type assertions), while unique write-operation coverage remains at least prior 132/135 baseline.
...
- [x] B04 Upgrade `api-real-smoke` from read-heavy to read-write balanced.
  - Acceptance: write coverage raised from 1.48% to >=20% (phase 1). (done: >=132/135 unique operations covered, currently 561 write assertions executed in smoke)
...
- [ ] E03 Add security tests for high-risk APIs (IDOR/guessing/replay/duplicate submit).
  ...
  - Progress: RBAC privilege-boundary smoke matrix landed (`report.read` allow + forbidden checks on listings/audit-logs/cases/maintenance/user-verifications/config/rbac/report-export), `requirePermission` deny path is normalized to 403 (not 500), and `BearerAuthGuard` no longer elevates `isAdmin` by arbitrary custom role assignment.
...
- Deliverables:
  ...
  39) RBAC privilege matrix deepening + permission deny-path normalization landed (done: custom role `report.read` now has explicit allow assertion on finance summary, forbidden assertions on high-risk admin endpoints, and `requirePermission` now returns consistent 403 instead of internal 500 on deny path).
