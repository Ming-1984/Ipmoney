...
- `smoke`: pass (API 1167/1167, UI HTTP 86/86, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 1167 checks (including semantic read-back/state assertions).
...
| B04 | done | Codex | 2026-03-06 | 2026-03-07 | `api-real-smoke` expanded to 1167/1167 (writes 858/858 + reads 309/309 including 93 semantic read-back assertions), with unique write-operation coverage already near-full baseline |
...
- Deliverables:
  1) `api-real-smoke` expanded from 17 to 1167 checks (done),
  ...
  51) next step: carry trend history across daily/CI runs and calibrate threshold factors using broader variance data (pending).
