...
- `smoke`: pass (API 1171/1171, UI HTTP 86/86, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 1171 checks (including semantic read-back/state assertions).
...
- [x] B04 Upgrade `api-real-smoke` from read-heavy to read-write balanced.
  - Acceptance: write coverage raised from 1.48% to >=20% (phase 1). (done: >=132/135 unique operations covered, currently 860 write assertions executed in smoke)
...
| B04 | done | Codex | 2026-03-06 | 2026-03-07 | `api-real-smoke` expanded to 1171/1171 (writes 860/860 + reads 311/311 including 93 semantic read-back assertions), with unique write-operation coverage already near-full baseline |
...
- Deliverables:
  ...
  52) next step: carry trend history across daily/CI runs and calibrate threshold factors using broader variance data (pending).
