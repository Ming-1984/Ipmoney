...
- `smoke`: pass (API 464/464, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- [x] B04 Upgrade `api-real-smoke` from read-heavy to read-write balanced.
  - Acceptance: write coverage raised from 1.48% to >=20% (phase 1). (done: >=132/135 unique operations covered, currently 372 write assertions executed in smoke)
...
| B04 | done | Codex | 2026-03-06 | 2026-03-06 | `api-real-smoke` expanded to 464/464 (writes 372/372 + reads 92/92 semantic assertions), with unique write-operation coverage already near-full baseline |
...
- Deliverables:
  ...
  30) AI/industry-tag/region/featured/file-temporary-access coverage landed (done: `POST /ai/agent/query`, `/admin/ai/parse-results` list/get/update branch coverage, `/ai/parse-results/:id/feedback` create + replay/missing checks, `POST /admin/industry-tags` create+duplicate-invalid checks + admin/public list visibility checks, `PUT /admin/regions/:code/industry-tags`, `PUT /admin/listings/:id/featured` + missing-id 404 regression guard, and `POST /files/:id/temporary-access` success + missing guards).
