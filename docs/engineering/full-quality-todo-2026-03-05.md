...
- `smoke`: pass (API 613/613, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 613 checks (including semantic read-back/state assertions).
- Write operations total 135; smoke now executes 520 write assertions plus 93 read-back semantic checks (state transitions, persistence, cross-module link integrity, same-key replay invariants, selected concurrency race outcomes, percentile-based chaos stability guard, cross-run trend-threshold capability, and AI/industry-tag/region/featured/file-temporary-access/admin-listings-write assertions), while unique write-operation coverage remains at least prior 132/135 baseline.
...
- [x] B04 Upgrade `api-real-smoke` from read-heavy to read-write balanced.
  - Acceptance: write coverage raised from 1.48% to >=20% (phase 1). (done: >=132/135 unique operations covered, currently 520 write assertions executed in smoke)
...
| B04 | done | Codex | 2026-03-06 | 2026-03-06 | `api-real-smoke` expanded to 613/613 (writes 520/520 + reads 93/93 semantic assertions), with unique write-operation coverage already near-full baseline |
...
- Deliverables:
  ...
  33) next step: carry trend history across daily/CI runs and calibrate threshold factors using broader variance data (pending).
