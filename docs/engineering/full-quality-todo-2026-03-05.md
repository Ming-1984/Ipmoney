...
- `smoke`: pass (API 101/101, UI HTTP 28/28, UI Render full 83/83, UI Render core 3/3, UI DOM core 11/11, UI DOM full-83 83/83).
...
- API smoke covers 101 operations (~41.6%).
- Write operations total 135; smoke now covers 73 (~54.1%).
...
- [ ] B02 Build write-first API test inventory (orders/refunds/invoices/comments/favorites/addresses/audit flow).
  ...
  - Progress: smoke batch now covers 73 write operations (favorites/comments/addresses/conversations/consultations + auth + admin-config writes + admin order/refund negative paths + order/admin happy-path state transitions + file-dependent payout/invoice paths), with failure/idempotency assertions and targeted regression checks added.
...
- [x] B04 Upgrade `api-real-smoke` from read-heavy to read-write balanced.
  - Acceptance: write coverage raised from 1.48% to >=20% (phase 1). (done: 73/135 ~= 54.1%)
...
| B04 | done | Codex | 2026-03-06 | 2026-03-06 | `api-real-smoke` expanded to 101/101 (writes 73/73), write coverage ~54.1% |
| B02 | in_progress | Codex | 2026-03-06 | - | write batch reached 73 ops and now includes file-dependent payout/invoice happy-paths; remaining depth is richer refund lifecycle and broader idempotency matrix |
...
- Deliverables:
  1) `api-real-smoke` expanded from 17 to 101 checks (done),
  2) write checks expanded from 2 to 73 (favorites/comments/addresses/conversations/consultations/auth/admin-config/admin-order-refund-negative/order-admin-happy-path/file-dependent payout-invoice) (done),
  ...
  9) next step: extend richer order/refund lifecycle + idempotency matrix (pending).
