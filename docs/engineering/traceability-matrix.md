...
| 检索（筛选/排序 + 分类切换） | 进入搜索主链路：专利交易｜成果展示｜机构；搜索条下新增排序 Chip 行（综合推荐/价格升序/价格降序/最新发布）；游客可看；关键动作需登录且审核通过 | `apps/client/src/pages/search/index.tsx` | `searchListings` `GET /search/listings`（游客或非推荐）；`getMyRecommendedListings` `GET /me/recommendations/listings`（登录态推荐）；`searchAchievements` `GET /search/achievements`；`listPublicOrganizations` `GET /public/organizations` | `GET /search/listings`；`GET /me/recommendations/listings`；`GET /search/achievements`；`GET /public/organizations` | P0 已实现 |
| 检索（更多筛选对齐） | Search 主链路更多筛选（专利/成果/机构：地区/行业/成熟度等）统一 `FilterSheet`；参数见 `docs/engineering/ui-guidelines.md` | `apps/client/src/pages/search/index.tsx` | 同上（仅增加 query 参数） | 同上（补齐更多筛选场景 fixtures） | P0 已实现 |
...
| 留言区（公开） | 专利/成果页面底部：公开留言列表 + 互动回复；留言/回复/编辑/删除需登录且审核通过（编辑/删除仅本人）；短按评论默认回复并滚动至输入栏，长按弹出“回复/编辑/删除”；未聚焦显示头像 + 占位文本“写下你的留言，共同讨论”，聚焦后在键盘上方显示输入栏 + 发布按钮（无内容灰色不可点，有内容点亮） | `apps/client/src/pages/listing/detail/index.tsx`<br/>`apps/client/src/pages/achievement/detail/index.tsx` | `listPublicListingComments` `GET /public/listings/{listingId}/comments`；`createListingComment` `POST /listings/{listingId}/comments`；`listPublicAchievementComments` `GET /public/achievements/{achievementId}/comments`；`createAchievementComment` `POST /achievements/{achievementId}/comments`；`updateComment` `PATCH /comments/{commentId}`；`deleteComment` `DELETE /comments/{commentId}` | fixtures 已覆盖（happy/empty/error/edge） | P0 已实现 |
...
| 发布入口（卖家侧） | 发布前需登录+身份选择；非个人需审核（专利/成果） | `apps/client/src/pages/publish/index.tsx` | `getMyVerification` `GET /me/verification`（前置校验） | `GET /me/verification` | P0 已实现 |
...
| 收藏 | 收藏/取消收藏/收藏列表（专利/成果） | `apps/client/src/pages/favorites/index.tsx` | `favoriteListing` `POST /listings/{listingId}/favorites`；`unfavoriteListing` `DELETE /listings/{listingId}/favorites`；`listMyFavoriteListings` `GET /me/favorites`；`favoriteAchievement` `POST /achievements/{achievementId}/favorites`；`unfavoriteAchievement` `DELETE /achievements/{achievementId}/favorites`；`listMyFavoriteAchievements` `GET /me/favorites/achievements` | `POST /listings/:listingId/favorites`；`DELETE /listings/:listingId/favorites`；`GET /me/favorites`；`POST /achievements/:achievementId/favorites`；`DELETE /achievements/:achievementId/favorites`；`GET /me/favorites/achievements` | P0 已实现 |
...
- P0 前端骨架已收口：收藏、机构详情、地图 drilldown、登录、买家订单/退款入口、交易规则前台展示等均已对齐 OpenAPI，并补齐 happy fixtures；技术经理人栏目已补齐并进入 P0 已实现。
- 仍属后端联调项：微信支付/退款回调验签与幂等（`POST /webhooks/wechatpay/notify`，前端不调用）。

### 3.1 OpenAPI 已有但前端暂未消费（需确认 P0/P1）

> 见 `docs/engineering/openapi-coverage.md`：部分新增 operations 仍未被前端消费（属计划内差异）；支付回调为后端内部接口，前端不调用。
...
| AI 解析卡片 + 评分 | 专利/成果详情展示 AI 解析并评分纠错 | `apps/client/src/pages/listing/detail/index.tsx`<br/>`apps/client/src/pages/achievement/detail/index.tsx` | `createAiParseFeedback` `POST /ai/parse-results/{parseResultId}/feedback` | `POST /ai/parse-results/:parseResultId/feedback` | P1（已落地基础版） |
| 平台自有内容 CMS | 后台创建/编辑/发布/下架挂牌/成果 | `apps/admin-web/src/views/ListingsAuditPage.tsx`<br/>`apps/admin-web/src/views/AchievementsAuditPage.tsx` | `adminCreateListing`/`adminUpdateListing`/`adminPublishListing`/`adminOffShelfListing`<br/>`adminCreateAchievement`/`adminUpdateAchievement`/`adminPublishAchievement`/`adminOffShelfAchievement` | fixtures 已覆盖（happy） | P1（已落地基础版） |
...
| 数据地图扩展 | 技术经理人/科学家地图 | `apps/client/src/pages/patent-map/index.tsx` | - | - | P1 |
| 大数据分析中心 | 交易库/成果库指标看板与导出 | `apps/admin-web/src/views/PlaceholderPage.tsx`（待补） | - | - | P1 |
