...
- [x] 专利详情页：概览/摘要/信息/评论合并为单页展示，不再跳转子页；挂牌详情页移除“关联专利/点击查看专利详情”，在挂牌详情页内嵌专利详情模块（摘要/附图/专利信息/权利人/说明书统计），避免重复展示
...
- [ ] 检索：`/search/listings`（游客可用）+ 过滤（类型/地区/标签/IPC/LOC/法律状态/价格/订金/转让次数）+ 排序（推荐/最新/价格升序/价格降序）
...
- [x] 检索（Search）：筛选区（专利类型/交易方式/价格类型/地域）+ 排序（推荐/最新/价格升序/价格降序）
- [x] 列表卡片（ListingCard）：价格/订金/标签/按钮样式统一；空态/错误态更友好
- [x] 详情页（Listing Detail）：首屏信息结构（标题/类型/价格/订金/卖家/权属材料）+ CTA（咨询/下单）
- [x] 留言区交互统一：短按评论默认回复；长按弹出“回复/编辑/删除”；输入框聚焦后在键盘上方显示输入栏+发布按钮（有内容点亮）
...
- [x] 12.3.2 首页（对外第一屏，优先“像产品”）
  ...
  - [x] 快捷入口：5 个一排（发明人榜/专利地图/书画专区/产学研需求/成果转化），移除“技术经理人”（由 TabBar「咨询」承载）
  - [x] 推荐区：展示“推荐分/地域特色/转让次数（沉睡专利）”标签；按钮与卡片密度统一
  - [x] 游客态文案：明确“可看/需登录且审核通过”（收藏/咨询/下单/支付）


- [x] 12.3.2.1 首页快捷入口按钮优化：发明人榜/专利地图/书画专区/产学研需求/成果转化（排版 + 尺寸；优先 NutUI `Grid` 收口）
- [x] 12.3.2.2 Home 副标题：`专利变金豆矿` → `专利点金台`
- [x] 12.3.3 特色专区：沉睡专利入口（跳转搜索）
  - [x] Segmented：推荐/最新（组件库）
  - [x] 卡片：地域特色置顶、行业标签、转让次数/沉睡专利标签更清晰
...
- [x] 排序：推荐/最新/价格升序/价格降序（组件库 Segmented/SortSheet）
...
- [x] 12.3.5 详情页（转化）
  - [x] 首屏结构：价格/订金/卖家/转让次数/标签（密度与层级优化）
  - [x] 风险提示：合同线下、尾款平台支付、变更完成后放款（卡片化）
  - [x] 底部 CTA：咨询 + 支付订金（金额/权限态文案）
  - [ ] 成果转化详情（成果展示）：首屏信息区包含标题、行业标签、应用阶段、所属单位、所在地、浏览量、发布时间；支持“立即咨询”
  - [ ] 成果媒体展示：发布者封面图 + 更多图片图集（可单独点开查看）
...
- [x] 12.3.9 身份注册/资料提交（首次进入）
  - [x] 身份选择：2 列卡片栅格化（图标居中/标题/说明/秒通过-需审核标签）
