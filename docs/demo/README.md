...
This folder stores **rendered demo artifacts** (screenshots/exports) for local QA.
They are **not** runtime dependencies and **not** production deployment assets.

## UI smoke screenshots (H5 + Admin Web)

- Output: `docs/demo/rendered/ui-smoke-YYYY-MM-DD/` (local only, git-ignored by default)
- Generate:
...
- If cleanup is needed, remove local screenshot folders under `docs/demo/rendered/`.
