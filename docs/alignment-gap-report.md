...
- [x] **列表卡片：补齐三类标签**
  - 验收：`ListingCard` 显示“沉睡专利/高新退役/产业集群特色”标签规则：
    ...
    - listingTopics 包含 HIGH_TECH_RETIRED 显示“高新退役”；
    - listingTopics 包含 CLUSTER_FEATURED 或 featuredLevel!=NONE 显示“产业集群”。
- [x] **书画卡片：仅保留信息展示**
...
- 新增上架审核“详情”抽屉入口，并接入材料/审计日志落库接口（材料来自文件关联，日志来自 audit_logs）。  
- 新增后端 listings 模块：后台列表/详情/通过/驳回/特色置顶。  
- 需求/成果/书画审核详情页接入材料/审计日志落库接口并展示。  
...
- 通知中心由静态数据切换为 /notifications 接口，接入内容审核/认证审核事件。  
...
- RBAC 接入权限校验与审计日志（占位）。  
- 配置中心接入权限校验与审计日志（占位）。  
- 专利地图 Excel 导入已在后端落库支持，与后台导入页对齐。  
...
- **临时兼容仍需清理**：后端已支持结构化字段（可交付清单/预计周期/可谈空间/质押/许可现状），前端发布可从摘要兜底切换为结构化字段（如仍保留旧逻辑）。
...
- **P0 待后端补齐字段**：专利发布“可交付资料清单、预计周期、可谈空间、质押/许可现状声明”等已在 PRD/ER/OpenAPI 补齐，待落库与接口实现。

### 5.3 仍需确认/延后的点（不阻塞 P0）
- 后台 P1 功能模块与权限细节（CMS/AI/告警/数据分析等）。
- H5 支付策略已确认：**仅引导回小程序支付**（P0 固定方案）。
...
- **缺失/偏差（P0）：**
  - **内容审核材料/审计日志**已接入落库接口（材料来自文件关联，日志来自 audit_logs），仍需文件权限/下载留痕优化。
...
- **注意：** 当前少量模块仍为“占位实现”（cases/rbac/reports/通知真实来源等）；其余核心模块已落库，可继续推进权限/审计/文件链路细化。
- **通知中心已接入审核/认证事件**；合同/发票已支持文件上传/下载与权限，仍需更细粒度审计/水印/临时 URL。

### 6.4 结论（是否“全面对齐”）
- **前端：P0 页面已对齐骨架与需求。**
- **后台：P0 关键页面基本对齐，材料/审计日志已接入落库接口，仍需文件权限/下载留痕优化。**
- **后端：P0 接口已补齐（少量占位），可进入后端正式开发与权限/审计/文件链路深化阶段。**
