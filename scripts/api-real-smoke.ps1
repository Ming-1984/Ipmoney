...
try {
  ...
  $adminDemandCreate = Add-ApiCaseResult -Results $results -Name "admin-demand-create" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/demands" -Body @{ title = $smokeAdminDemandTitle; source = "ADMIN"; status = "DRAFT"; auditStatus = "PENDING"; budgetType = "FIXED"; budgetMinFen = 1000; budgetMaxFen = 3000; deliveryPeriod = "MONTH_1_3"; industryTags = @($newIndustryTagName) } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-demand-create") -Expected @(200, 201)
  ...
  $adminDemandUpdate = Add-ApiCaseResult -Results $results -Name "admin-demand-update" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/demands/$smokeAdminDemandId" -Body @{ title = "$smokeAdminDemandTitle Updated"; source = "PLATFORM"; status = "ACTIVE"; auditStatus = "APPROVED"; budgetType = "NEGOTIABLE"; deliveryPeriod = "WITHIN_1_MONTH"; industryTags = @($newIndustryTagName) } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-demand-update") -Expected @(200)
  ...
  $adminAchievementUpdate = Add-ApiCaseResult -Results $results -Name "admin-achievement-update" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/$smokeAdminAchievementId" -Body @{ title = "$smokeAdminAchievementTitle Updated"; source = "PLATFORM"; status = "ACTIVE"; auditStatus = "APPROVED"; maturity = "PILOT"; industryTags = @($newIndustryTagName) } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-update") -Expected @(200)
