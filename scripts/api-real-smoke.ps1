...
try {
  ...
  if ($adminNonSelfOrderableListings.Count -le 0) {
    $fallbackSellerUserId = [string]$env:DEMO_ADMIN_ID
    if ([string]::IsNullOrWhiteSpace($fallbackSellerUserId)) {
      throw "DEMO_ADMIN_ID is required to prepare cross-seller orderable listing"
    }
    $fallbackOrderableTitle = "Smoke Cross Seller Orderable Listing $ReportDate $([guid]::NewGuid().ToString('N').Substring(0, 8))"
    $fallbackOrderableCreate = Add-ApiCaseResult -Results $results -Name "admin-listing-prepare-orderable-cross-seller-create" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings" -Body @{ title = $fallbackOrderableTitle; sellerUserId = $fallbackSellerUserId; source = "ADMIN"; status = "DRAFT"; auditStatus = "PENDING"; tradeMode = "LICENSE"; licenseMode = "EXCLUSIVE"; priceType = "FIXED"; priceAmountFen = 888000; depositAmountFen = 1000; pledgeStatus = "NONE"; existingLicenseStatus = "SOLE"; regionCode = $importRegionCode } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-prepare-orderable-cross-seller-create") -Expected @(200, 201)
    ...
    if ([string]::IsNullOrWhiteSpace($fallbackOrderableListingId)) {
      throw "admin-listing-prepare-orderable-cross-seller-create missing id"
    }
    [void](Add-ApiCaseResult -Results $results -Name "admin-listing-prepare-orderable-cross-seller-approve" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$fallbackOrderableListingId/approve" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-prepare-orderable-cross-seller-approve") -Expected @(200, 201, 409))
    [void](Add-ApiCaseResult -Results $results -Name "admin-listing-prepare-orderable-cross-seller-publish" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$fallbackOrderableListingId/publish" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-prepare-orderable-cross-seller-publish") -Expected @(200, 201, 409))
    $fallbackOrderableListingDetail = Add-ApiCaseResult -Results $results -Name "admin-listing-prepare-orderable-cross-seller-get" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$fallbackOrderableListingId" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200)
  ...
  if ($adminNonSelfOrderableListings.Count -gt 0) {
    $selectedListing = @($adminNonSelfOrderableListings | Select-Object -First 1)[0]
  } elseif ($nonSelfOrderableListings.Count -gt 0) {
  ...
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-request-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/invoice-requests" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-invoice-request-invalid-order-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-request-not-completed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice-requests" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-invoice-request-not-completed") -Expected @(409))
