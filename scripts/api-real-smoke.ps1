...
function Assert-LoopbackServiceUrl {
  param(
    [string]$Name,
    [string]$Url
  )
  ...
  if ($uri.Host -notin @('localhost', '127.0.0.1', '::1')) {
    throw "$Name must use a loopback host for write smoke tests; received host '$($uri.Host)'"
  }
}

Assert-LoopbackServiceUrl -Name 'DATABASE_URL' -Url $DatabaseUrl
Assert-LoopbackServiceUrl -Name 'REDIS_URL' -Url $RedisUrl
...
function Assert-ResultJsonArrayNotContains {
  param(
    [pscustomobject]$Result,
    [string]$Field,
  ...
  if ($items -contains [string]$UnexpectedValue) {
    Add-ResultAssertionFailure -Result $Result -Assertion $Assertion -Message "Array '$Field' should not contain '$UnexpectedValue'"
  }
}
...
function New-OrderableListingForScenario {
  param(
    [System.Collections.ArrayList]$Results,
    [int]$ApiPort,
    [string]$UserToken,
  ...
  $regionCode = [string]$script:importRegionCode
  $sellerToken = [string]$script:scenarioSellerToken
  $proofFileId = [string]$script:scenarioSellerProofFileId
  if ([string]::IsNullOrWhiteSpace($regionCode)) { throw "$CasePrefix-regionCode is required" }
  if ([string]::IsNullOrWhiteSpace($sellerToken)) { throw "$CasePrefix-sellerToken is required" }
  if ([string]::IsNullOrWhiteSpace($proofFileId)) { throw "$CasePrefix-proofFileId is required" }

  $title = "Smoke Scenario Listing $CasePrefix $([guid]::NewGuid().ToString('N').Substring(0, 8))"
  $create = Add-ApiCaseResult -Results $Results -Name "$CasePrefix-listing-create" -Method "POST" -Url "http://127.0.0.1:$ApiPort/listings" -Body @{ title = $title; tradeMode = "LICENSE"; licenseMode = "EXCLUSIVE"; priceType = "FIXED"; priceAmountFen = 888000; depositAmountFen = 1000; pledgeStatus = "NONE"; existingLicenseStatus = "SOLE"; regionCode = $regionCode; proofFileIds = @($proofFileId) } -Headers (New-WriteHeaders -AuthorizationToken $sellerToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-listing-create") -Expected @(200, 201)
  $listingId = Get-ResultStringField -Result $create -Field "id"
  if ([string]::IsNullOrWhiteSpace($listingId)) { throw "$CasePrefix-listing-create missing id" }
  [void](Add-ApiCaseResult -Results $Results -Name "$CasePrefix-listing-submit" -Method "POST" -Url "http://127.0.0.1:$ApiPort/listings/$listingId/submit" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $sellerToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-listing-submit") -Expected @(200, 201))
  $approve = Add-ApiCaseResult -Results $Results -Name "$CasePrefix-listing-approve" -Method "POST" -Url "http://127.0.0.1:$ApiPort/admin/listings/$listingId/approve" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $AdminToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-listing-approve") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $approve -Field "status" -ExpectedValue "ACTIVE" -Assertion "$CasePrefix-listing-approve-status"
  Assert-ResultJsonFieldEquals -Result $approve -Field "auditStatus" -ExpectedValue "APPROVED" -Assertion "$CasePrefix-listing-approve-audit-status"
  return $listingId
}
...
function New-RefundReadyOrder {
  ...
  $contractFileId = [string]$script:scenarioSellerContractPdfFileId
  $sellerToken = [string]$script:scenarioSellerToken
  if ([string]::IsNullOrWhiteSpace($contractFileId)) { throw "$CasePrefix-contract-file is required" }
  if ([string]::IsNullOrWhiteSpace($sellerToken)) { throw "$CasePrefix-seller-token is required" }
  $scenarioListingId = New-OrderableListingForScenario -Results $Results -ApiPort $ApiPort -UserToken $UserToken -AdminToken $AdminToken -IdempotencyPrefix $IdempotencyPrefix -CasePrefix $CasePrefix
  $orderCreate = Add-ApiCaseResult -Results $Results -Name "$CasePrefix-order-create" -Method "POST" -Url "http://127.0.0.1:$ApiPort/orders" -Body @{ listingId = $scenarioListingId } -Headers (New-WriteHeaders -AuthorizationToken $UserToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-order-create") -Expected @(200, 201)
  ...
  [void](Add-ApiCaseResult -Results $Results -Name "$CasePrefix-admin-order-contract-upload" -Method "POST" -Url "http://127.0.0.1:$ApiPort/admin/orders/$orderId/contract/upload" -Body @{ contractFileId = $contractFileId } -Headers (New-WriteHeaders -AuthorizationToken $AdminToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-admin-order-contract-upload") -Expected @(200, 201))
  $signedSubmission = Add-ApiCaseResult -Results $Results -Name "$CasePrefix-contract-signed-submission" -Method "POST" -Url "http://127.0.0.1:$ApiPort/contracts/contract-$orderId/signed-submissions" -Body @{ fileId = $contractFileId } -Headers (New-WriteHeaders -AuthorizationToken $sellerToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-contract-signed-submission") -Expected @(200, 201)
  $signedSubmissionId = Get-ResultStringField -Result $signedSubmission -Field "id"
  if ([string]::IsNullOrWhiteSpace($signedSubmissionId)) { throw "$CasePrefix-contract-signed-submission missing id" }
  [void](Add-ApiCaseResult -Results $Results -Name "$CasePrefix-admin-order-contract-signed" -Method "POST" -Url "http://127.0.0.1:$ApiPort/admin/orders/$orderId/milestones/contract-signed" -Body @{ dealAmountFen = 2000000; signedSubmissionId = $signedSubmissionId } -Headers (New-WriteHeaders -AuthorizationToken $AdminToken -Prefix $IdempotencyPrefix -Label "$CasePrefix-admin-order-contract-signed") -Expected @(200, 201))
  return $orderId
}
...
# Smoke must never send a real SMS, even when a local .env configures a provider.
...
try {
  ...
  $invoiceRequestBody = @{ titleType = "PERSONAL"; titleName = "Smoke Invoice Buyer" }
  ...
  $cases = @(
    ...
    @{ name = "me-verification-empty-region-code"; method = "POST"; url = "http://127.0.0.1:$resolvedApiPort/me/verification"; body = @{ type = "PERSON"; displayName = "Smoke Empty Verification Region"; regionCode = "" }; headers = @{ Authorization = $userToken }; expected = @(400) },
    @{ name = "me-verification-empty-logo-file-id"; method = "POST"; url = "http://127.0.0.1:$resolvedApiPort/me/verification"; body = @{ type = "PERSON"; displayName = "Smoke Empty Verification Logo"; logoFileId = "" }; headers = @{ Authorization = $userToken }; expected = @(400) },
    @{ name = "me-verification-empty-unified-social-credit-code"; method = "POST"; url = "http://127.0.0.1:$resolvedApiPort/me/verification"; body = @{ type = "PERSON"; displayName = "Smoke Empty Verification USCC"; unifiedSocialCreditCode = "" }; headers = @{ Authorization = $userToken }; expected = @(400) },
    @{ name = "me-verification-empty-evidence-file-id"; method = "POST"; url = "http://127.0.0.1:$resolvedApiPort/me/verification"; body = @{ type = "PERSON"; displayName = "Smoke Empty Verification Evidence"; evidenceFileIds = @("") }; headers = @{ Authorization = $userToken }; expected = @(400) },
    ...
    @{ name = "notifications-detail-invalid-notification-id-format"; method = "GET"; url = "http://127.0.0.1:$resolvedApiPort/notifications/not-a-uuid"; body = $null; headers = @{ Authorization = $userToken }; expected = @(400) },
    ...
    @{ name = "admin-config-home-landing-get"; method = "GET"; url = "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing"; body = $null; headers = @{ Authorization = $adminToken }; expected = @(200) },
    ...
    @{ name = "public-config-home-landing"; method = "GET"; url = "http://127.0.0.1:$resolvedApiPort/public/config/home-landing"; body = $null; headers = @{}; expected = @(200) },
  ...
  # Concurrent order scenarios need a normal buyer and a separate verified seller.
  $scenarioSellerPhone = "139$((Get-Random -Minimum 10000000 -Maximum 99999999).ToString())"
  $scenarioSellerSmsSend = Add-ApiCaseResult -Results $results -Name "scenario-seller-auth-sms-send" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/auth/sms/send" -Body @{ phone = $scenarioSellerPhone; purpose = "LOGIN" } -Headers @{} -Expected @(200, 201)
  $scenarioSellerSmsCode = Get-ResultStringField -Result $scenarioSellerSmsSend -Field "debugCode"
  if ([string]::IsNullOrWhiteSpace($scenarioSellerSmsCode)) { throw "scenario-seller-auth-sms-send missing debugCode" }
  $scenarioSellerLogin = Add-ApiCaseResult -Results $results -Name "scenario-seller-auth-sms-verify" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/auth/sms/verify" -Body @{ phone = $scenarioSellerPhone; code = $scenarioSellerSmsCode } -Headers @{} -Expected @(200, 201)
  $scenarioSellerAccessToken = Get-ResultStringField -Result $scenarioSellerLogin -Field "accessToken"
  if ([string]::IsNullOrWhiteSpace($scenarioSellerAccessToken)) { throw "scenario-seller-auth-sms-verify missing accessToken" }
  $scenarioSellerToken = "Bearer $scenarioSellerAccessToken"
  $scenarioSellerVerification = Add-ApiCaseResult -Results $results -Name "scenario-seller-verification" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/me/verification" -Body @{ type = "PERSON"; displayName = "Smoke Scenario Seller" } -Headers (New-WriteHeaders -AuthorizationToken $scenarioSellerToken -Prefix $idempotencyPrefix -Label "scenario-seller-verification") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $scenarioSellerVerification -Field "status" -ExpectedValue "APPROVED" -Assertion "scenario-seller-verification-status"
  $scenarioSellerProofUpload = Add-ApiFileUploadCaseResult -Results $results -Name "scenario-seller-proof-upload" -Url "http://127.0.0.1:$resolvedApiPort/files" -AuthorizationToken $scenarioSellerToken -FilePath $smokeEvidencePath -FormFields $null -Expected @(200, 201)
  $scenarioSellerProofFileId = Get-ResultStringField -Result $scenarioSellerProofUpload -Field "id"
  if ([string]::IsNullOrWhiteSpace($scenarioSellerProofFileId)) { throw "scenario-seller-proof-upload missing id" }
  $scenarioSellerContractUpload = Add-ApiFileUploadCaseResult -Results $results -Name "scenario-seller-contract-upload" -Url "http://127.0.0.1:$resolvedApiPort/files" -AuthorizationToken $scenarioSellerToken -FilePath $smokeContractPdfPath -FormFields $null -Expected @(200, 201)
  $scenarioSellerContractPdfFileId = Get-ResultStringField -Result $scenarioSellerContractUpload -Field "id"
  if ([string]::IsNullOrWhiteSpace($scenarioSellerContractPdfFileId)) { throw "scenario-seller-contract-upload missing id" }
  ...
  $adminHomeLandingConfig = Invoke-RestMethod -Method Get -Uri "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing" -Headers @{ Authorization = $adminToken }
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-logo-update-invalid-verification-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/not-a-uuid/logo" -Body @{ logoFileId = $null } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-user-verification-logo-update-invalid-verification-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-logo-update-missing-logo-file-id" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/$([guid]::NewGuid().ToString())/logo" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-user-verification-logo-update-missing-logo-file-id") -Expected @(400))
  ...
  $adminUserId = [string]$env:DEMO_ADMIN_ID
  $adminNonSelfOrderableListings = @($nonSelfOrderableListings | Where-Object { $_.sellerUserId -and [string]$_.sellerUserId -eq $adminUserId })
  $smokeTokenOrderableListings = @($orderableListings | Where-Object { $_.sellerUserId -and ([string]$_.sellerUserId -eq $currentUserId -or [string]$_.sellerUserId -eq $adminUserId) })
  $fallbackOrderableListingJson = $null
  # Always create a dedicated listing so every write uses known, disposable identities.
  $createDedicatedOrderableListing = $true
  if ($createDedicatedOrderableListing) {
    $fallbackSellerUserId = $currentUserId
    if ([string]::IsNullOrWhiteSpace($fallbackSellerUserId)) {
      throw "Current smoke user id is required to prepare orderable listing"
    }
    $fallbackOrderableTitle = "Smoke Orderable Listing $ReportDate $([guid]::NewGuid().ToString('N').Substring(0, 8))"
    $fallbackOrderableProofUpload = Add-ApiFileUploadCaseResult -Results $results -Name "listing-prepare-orderable-proof-upload" -Url "http://127.0.0.1:$resolvedApiPort/files" -AuthorizationToken $userToken -FilePath $smokeEvidencePath -FormFields $null -Expected @(200, 201)
    $fallbackOrderableProofFileId = Get-ResultStringField -Result $fallbackOrderableProofUpload -Field "id"
    if ([string]::IsNullOrWhiteSpace($fallbackOrderableProofFileId)) {
      throw "listing-prepare-orderable-proof-upload missing id"
    }
    $fallbackOrderableCreate = Add-ApiCaseResult -Results $results -Name "listing-prepare-orderable-create" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings" -Body @{ title = $fallbackOrderableTitle; tradeMode = "LICENSE"; licenseMode = "EXCLUSIVE"; priceType = "FIXED"; priceAmountFen = 888000; depositAmountFen = 1000; pledgeStatus = "NONE"; existingLicenseStatus = "SOLE"; regionCode = $importRegionCode; proofFileIds = @($fallbackOrderableProofFileId) } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-prepare-orderable-create") -Expected @(200, 201)
    $fallbackOrderableListingId = Get-ResultStringField -Result $fallbackOrderableCreate -Field "id"
    if ([string]::IsNullOrWhiteSpace($fallbackOrderableListingId)) {
      throw "admin-listing-prepare-orderable-create missing id"
    }
    [void](Add-ApiCaseResult -Results $results -Name "listing-prepare-orderable-submit" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$fallbackOrderableListingId/submit" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-prepare-orderable-submit") -Expected @(200, 201))
    $fallbackOrderableApprove = Add-ApiCaseResult -Results $results -Name "admin-listing-prepare-orderable-approve" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$fallbackOrderableListingId/approve" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-prepare-orderable-approve") -Expected @(200, 201)
    Assert-ResultJsonFieldEquals -Result $fallbackOrderableApprove -Field "status" -ExpectedValue "ACTIVE" -Assertion "listing-prepare-orderable-approve-status"
    Assert-ResultJsonFieldEquals -Result $fallbackOrderableApprove -Field "auditStatus" -ExpectedValue "APPROVED" -Assertion "listing-prepare-orderable-approve-audit-status"
    $fallbackOrderableListingDetail = Add-ApiCaseResult -Results $results -Name "admin-listing-prepare-orderable-get" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$fallbackOrderableListingId" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200)
    $fallbackOrderableListingJson = Get-ResultJsonObject -Result $fallbackOrderableListingDetail
    if ($fallbackOrderableListingJson) {
      $listingCandidates = @($listingCandidates + @($fallbackOrderableListingJson))
      $orderableListings = @(
        $listingCandidates | Where-Object {
          [string]$_.status -eq "ACTIVE" -and
          [string]$_.auditStatus -eq "APPROVED" -and
  ...
  if ($fallbackOrderableListingJson) {
    $selectedListing = $fallbackOrderableListingJson
  } elseif ($adminNonSelfOrderableListings.Count -gt 0) {
    $selectedListing = @($adminNonSelfOrderableListings | Select-Object -First 1)[0]
  } elseif ($smokeTokenOrderableListings.Count -gt 0) {
    $selectedListing = @($smokeTokenOrderableListings | Select-Object -First 1)[0]
  } else {
    throw "No orderable listing mapped to smoke tokens was prepared"
  }
  $listingId = [string]$selectedListing.id
  if ([string]::IsNullOrWhiteSpace($listingId)) {
    throw "No valid listing id available for smoke write/order cases"
  }
  $selectedListingSellerUserId = [string]$selectedListing.sellerUserId
  if ([string]::IsNullOrWhiteSpace($selectedListingSellerUserId)) {
    throw "Selected listing sellerUserId is empty"
  }
  if ($selectedListingSellerUserId -ne $currentUserId -and $selectedListingSellerUserId -ne $adminUserId) {
    throw "Selected listing sellerUserId '$selectedListingSellerUserId' is not mapped to smoke tokens"
  }
  $orderBuyerToken = $userToken
  $orderSellerToken = $userToken
  if ($selectedListingSellerUserId -eq $adminUserId) {
    $orderBuyerToken = $userToken
    $orderSellerToken = $adminToken
  } elseif ($selectedListingSellerUserId -eq $currentUserId) {
    $orderBuyerToken = $adminToken
    $orderSellerToken = $userToken
  }
  $orderNonSellerToken = if ($orderSellerToken -eq $adminToken) { $userToken } else { $adminToken }
  if ([string]$selectedListing.auditStatus -ne "APPROVED" -or [string]$selectedListing.status -ne "ACTIVE") {
    throw "Selected orderable listing '$listingId' is not ACTIVE/APPROVED"
  }
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-tech-managers-batch-rating-invalid-body" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/tech-managers/batch/rating" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-tech-managers-batch-rating-invalid-body") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "public-listing-detail-invalid-listing-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/public/listings/not-a-uuid" -Body $null -Headers @{} -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-create-empty-negotiable-note" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings" -Body @{ title = "Smoke User Listing Empty Negotiable Note"; negotiableNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-create-empty-negotiable-note") -Expected @(200, 201))
  [void](Add-ApiCaseResult -Results $results -Name "listing-create-empty-encumbrance-note" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings" -Body @{ title = "Smoke User Listing Empty Encumbrance Note"; encumbranceNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-create-empty-encumbrance-note") -Expected @(200, 201))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-update-empty-title" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/listings/$smokeUserListingId" -Body @{ title = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-update-empty-title") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "listing-update-empty-summary" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/listings/$smokeUserListingId" -Body @{ summary = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-update-empty-summary") -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-update-empty-negotiable-note" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/listings/$smokeUserListingId" -Body @{ negotiableNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-update-empty-negotiable-note") -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "listing-update-empty-encumbrance-note" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/listings/$smokeUserListingId" -Body @{ encumbranceNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-update-empty-encumbrance-note") -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-get-mine-invalid-listing-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid" -Body $null -Headers @{ Authorization = $userToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "listing-update-invalid-listing-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid" -Body @{ title = "smoke invalid listing id format update" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-update-invalid-listing-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "listing-submit-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid/submit" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-submit-invalid-listing-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "listing-off-shelf-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid/off-shelf" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-off-shelf-invalid-listing-id-format") -Expected @(400))
  ...
  foreach ($adminIndustryTagItem in $adminIndustryTagsAfterCreateItems) {
    ...
    if ($adminIndustryTagItem -and $adminIndustryTagItem.name -and [string]$adminIndustryTagItem.name -eq $newIndustryTagNameAlt) {
      $adminIndustryTagCreatedAltVisible = $true
    }
  ...
  if (-not $adminIndustryTagCreatedAltVisible) {
    Add-ResultAssertionFailure -Result $adminIndustryTagsAfterCreate -Assertion "admin-industry-tags-created-alt-visible" -Message "Created industry tag '$newIndustryTagNameAlt' not found in admin list"
  }
  ...
  if ($publicIndustryTagNames -contains $newIndustryTagNameAlt) {
    Add-ResultAssertionFailure -Result $publicIndustryTagsList -Assertion "public-industry-tags-smoke-tag-underscore-hidden" -Message "Public industry tags should not expose test tag '$newIndustryTagNameAlt'"
  }
  ...
  if ([int]$aiAgentQueryWithHiddenIndustryTag.status -eq 200) {
    ...
    if ($aiAgentHiddenJson) {
      ...
      if ($aiIndustryTagsLookup.found -and $aiIndustryTagsLookup.value -is [System.Collections.IEnumerable] -and -not ($aiIndustryTagsLookup.value -is [string])) {
        ...
        if ($aiIndustryTags -contains $hiddenIndustryTagFilterAlt) {
          Add-ResultAssertionFailure -Result $aiAgentQueryWithHiddenIndustryTag -Assertion "ai-agent-query-hidden-industry-filter-alt-hidden" -Message "AI parsed filters should not contain hidden test industry tag '$hiddenIndustryTagFilterAlt'"
        }
  ...
  if (-not [string]::IsNullOrWhiteSpace($aiParseResultId)) {
    ...
    [void](Add-ApiCaseResult -Results $results -Name "admin-ai-parse-result-get-invalid-parse-result-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/ai/parse-results/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
    [void](Add-ApiCaseResult -Results $results -Name "admin-ai-parse-result-update-invalid-parse-result-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/ai/parse-results/not-a-uuid" -Body @{ status = "ACTIVE"; note = "smoke invalid parse result id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-ai-parse-result-update-invalid-parse-result-id-format") -Expected @(400, 404))
    [void](Add-ApiCaseResult -Results $results -Name "ai-parse-feedback-invalid-parse-result-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/ai/parse-results/not-a-uuid/feedback" -Body @{ score = 4; reasonTags = @("SMOKE"); comment = "smoke invalid parse result id format" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "ai-parse-feedback-invalid-parse-result-id-format") -Expected @(400, 404))
  }
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-get-by-id-invalid-patent-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "patent-get-by-id-invalid-patent-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/patents/not-a-uuid" -Body $null -Headers @{ Authorization = $userToken } -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-create-empty-title" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = "2026123456691"; patentType = "INVENTION"; title = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-create-empty-title") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-create-empty-application-no-display" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = "2026123456721"; patentType = "INVENTION"; title = "Smoke Patent Empty Application No Display"; applicationNoDisplay = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-create-empty-application-no-display") -Expected @(200, 201))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-create-empty-legal-status" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = "2026123456731"; patentType = "INVENTION"; title = "Smoke Patent Empty Legal Status"; legalStatus = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-create-empty-legal-status") -Expected @(200, 201))
  ...
  $smokePatentUpsertAbstractValue = "smoke abstract keep $ReportDate"
  $adminPatentUpsertAbstractSeed = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-abstract-seed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert Abstract Seed"; abstract = $smokePatentUpsertAbstractValue } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-abstract-seed") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertAbstractSeed -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-abstract-seed-id"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertAbstractSeed -Field "abstract" -ExpectedValue $smokePatentUpsertAbstractValue -Assertion "admin-patent-upsert-abstract-seed-value"
  $adminPatentUpsertAbstractPreserveOnOmit = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-abstract-preserve-on-omit" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert Abstract Preserve" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-abstract-preserve-on-omit") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertAbstractPreserveOnOmit -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-abstract-preserve-on-omit-id"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertAbstractPreserveOnOmit -Field "abstract" -ExpectedValue $smokePatentUpsertAbstractValue -Assertion "admin-patent-upsert-abstract-preserve-on-omit-value"
  $adminPatentUpsertAbstractClearNull = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-abstract-clear-null" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert Abstract Clear"; abstract = $null } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-abstract-clear-null") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertAbstractClearNull -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-abstract-clear-null-id"
  Assert-ResultJsonFieldMissing -Result $adminPatentUpsertAbstractClearNull -Field "abstract" -Assertion "admin-patent-upsert-abstract-clear-null-value"
  $smokePatentUpsertApplicationNoDisplayValue = "SMOKE-APPNO-$smokePatentSeq"
  $adminPatentUpsertApplicationNoDisplaySeed = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-application-no-display-seed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert AppNoDisplay Seed"; applicationNoDisplay = $smokePatentUpsertApplicationNoDisplayValue } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-application-no-display-seed") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertApplicationNoDisplaySeed -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-application-no-display-seed-id"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertApplicationNoDisplaySeed -Field "applicationNoDisplay" -ExpectedValue $smokePatentUpsertApplicationNoDisplayValue -Assertion "admin-patent-upsert-application-no-display-seed-value"
  $adminPatentUpsertApplicationNoDisplayPreserveOnOmit = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-application-no-display-preserve-on-omit" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert AppNoDisplay Preserve" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-application-no-display-preserve-on-omit") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertApplicationNoDisplayPreserveOnOmit -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-application-no-display-preserve-on-omit-id"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertApplicationNoDisplayPreserveOnOmit -Field "applicationNoDisplay" -ExpectedValue $smokePatentUpsertApplicationNoDisplayValue -Assertion "admin-patent-upsert-application-no-display-preserve-on-omit-value"
  $adminPatentUpsertApplicationNoDisplayClearNull = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-application-no-display-clear-null" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert AppNoDisplay Clear"; applicationNoDisplay = $null } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-application-no-display-clear-null") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertApplicationNoDisplayClearNull -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-application-no-display-clear-null-id"
  Assert-ResultJsonFieldMissing -Result $adminPatentUpsertApplicationNoDisplayClearNull -Field "applicationNoDisplay" -Assertion "admin-patent-upsert-application-no-display-clear-null-value"
  $smokePatentUpsertSourceUpdatedAtValue = "2020-01-02T03:04:05.000Z"
  $adminPatentUpsertSourceMetaSeed = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-source-meta-seed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert Source Meta Seed"; sourcePrimary = "PROVIDER"; sourceUpdatedAt = $smokePatentUpsertSourceUpdatedAtValue } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-source-meta-seed") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertSourceMetaSeed -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-source-meta-seed-id"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertSourceMetaSeed -Field "sourcePrimary" -ExpectedValue "PROVIDER" -Assertion "admin-patent-upsert-source-meta-seed-primary"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertSourceMetaSeed -Field "sourceUpdatedAt" -ExpectedValue $smokePatentUpsertSourceUpdatedAtValue -Assertion "admin-patent-upsert-source-meta-seed-updated-at"
  $adminPatentUpsertSourceMetaPreserveOnOmit = Add-ApiCaseResult -Results $results -Name "admin-patent-upsert-source-meta-preserve-on-omit" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents" -Body @{ applicationNoNorm = $smokePatentApplicationNoNorm; patentType = "INVENTION"; title = "$smokePatentTitle Upsert Source Meta Preserve" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-upsert-source-meta-preserve-on-omit") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertSourceMetaPreserveOnOmit -Field "id" -ExpectedValue $smokePatentId -Assertion "admin-patent-upsert-source-meta-preserve-on-omit-id"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertSourceMetaPreserveOnOmit -Field "sourcePrimary" -ExpectedValue "PROVIDER" -Assertion "admin-patent-upsert-source-meta-preserve-on-omit-primary"
  Assert-ResultJsonFieldEquals -Result $adminPatentUpsertSourceMetaPreserveOnOmit -Field "sourceUpdatedAt" -ExpectedValue $smokePatentUpsertSourceUpdatedAtValue -Assertion "admin-patent-upsert-source-meta-preserve-on-omit-updated-at"
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-empty-filing-date" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/$smokePatentId" -Body @{ filingDate = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-empty-filing-date") -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-invalid-patent-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/not-a-uuid" -Body @{ title = "Smoke invalid patent id format update" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-invalid-patent-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-empty-title" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/$smokePatentId" -Body @{ title = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-empty-title") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-null-title" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/$smokePatentId" -Body @{ title = $null } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-null-title") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-empty-application-no-display" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/$smokePatentId" -Body @{ applicationNoDisplay = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-empty-application-no-display") -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-empty-source-updated-at" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/$smokePatentId" -Body @{ sourceUpdatedAt = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-empty-source-updated-at") -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-update-empty-legal-status" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/$smokePatentId" -Body @{ legalStatus = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-update-empty-legal-status") -Expected @(200))
  ...
  if ($regionIndustryTags.Count -gt 0) {
    ...
    $publicRegionsAfterIndustryTagsSet = Add-ApiCaseResult -Results $results -Name "public-regions-list-after-industry-tags-set" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/regions?level=PROVINCE" -Body $null -Headers @{} -Expected @(200)
    ...
    $publicImportRegionAfterIndustryTagsSet = @(
      $publicRegionsAfterIndustryTagsSetItems |
        Where-Object {
          [string]$_.code -eq $importRegionCode -or
          [string]$_.regionCode -eq $importRegionCode
        } |
        Select-Object -First 1
    )[0]
    if (-not $publicImportRegionAfterIndustryTagsSet) {
      ...
      if ($publicImportRegionIndustryTags -contains $newIndustryTagNameAlt) {
        Add-ResultAssertionFailure -Result $publicRegionsAfterIndustryTagsSet -Assertion "public-regions-industry-tags-smoke-tag-underscore-hidden" -Message "Public regions should not expose test industry tag '$newIndustryTagNameAlt' in region '$importRegionCode'"
      }
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-get-by-id-invalid-listing-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-materials-invalid-listing-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid/materials" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-audit-logs-invalid-listing-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid/audit-logs" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "listing-get-by-id-invalid-listing-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid" -Body $null -Headers @{ Authorization = $userToken } -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-create-empty-seller-user-id" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings" -Body @{ title = "Smoke Admin Listing Empty Seller"; sellerUserId = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-create-empty-seller-user-id") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-create-empty-title" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings" -Body @{ title = ""; sellerUserId = $currentUserId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-create-empty-title") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-create-empty-summary" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings" -Body @{ title = "Smoke Admin Listing Empty Summary"; sellerUserId = $currentUserId; summary = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-create-empty-summary") -Expected @(200, 201))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-create-empty-negotiable-note" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings" -Body @{ title = "Smoke Admin Listing Empty Negotiable Note"; sellerUserId = $currentUserId; negotiableNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-create-empty-negotiable-note") -Expected @(200, 201))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-create-empty-encumbrance-note" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings" -Body @{ title = "Smoke Admin Listing Empty Encumbrance Note"; sellerUserId = $currentUserId; encumbranceNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-create-empty-encumbrance-note") -Expected @(200, 201))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-update-restore-region-code" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$smokeAdminListingId" -Body @{ regionCode = $importRegionCode } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-update-restore-region-code") -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-update-invalid-listing-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid" -Body @{ title = "smoke invalid listing id format update" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-update-invalid-listing-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-update-empty-negotiable-note" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$smokeAdminListingId" -Body @{ negotiableNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-update-empty-negotiable-note") -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-update-empty-encumbrance-note" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$smokeAdminListingId" -Body @{ encumbranceNote = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-update-empty-encumbrance-note") -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-off-shelf-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid/off-shelf" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-off-shelf-invalid-listing-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-publish-before-approval" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/$smokeAdminListingId/publish" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-publish-before-approval") -Expected @(409))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-publish-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid/publish" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-publish-invalid-listing-id-format") -Expected @(400))
  ...
  $publicListingSearchWithHiddenIndustryFilterAlt = Add-ApiCaseResult -Results $results -Name "public-listing-search-hidden-industry-filter-alt-underscore-ignored" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/search/listings?q=$publicListingSearchQuery&industryTags=$encodedHiddenIndustryTagFilterAlt&page=1&pageSize=20" -Body $null -Headers @{} -Expected @(200)
  Assert-ResultJsonItemsContainsId -Result $publicListingSearchWithHiddenIndustryFilterAlt -ItemId $smokeAdminListingId -Assertion "public-listing-search-hidden-industry-filter-alt-item-visible"
  Assert-ResultJsonItemsItemArrayNotContains -Result $publicListingSearchWithHiddenIndustryFilterAlt -ItemId $smokeAdminListingId -ArrayField "industryTags" -UnexpectedValue $newIndustryTagNameAlt -Assertion "public-listing-search-hidden-industry-filter-alt-smoke-tag-hidden"
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-reject-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/not-a-uuid/reject" -Body @{ reason = "smoke invalid listing id format reject" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-reject-invalid-listing-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-favorite-post-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid/favorites" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "favorite-listing-post-invalid-listing-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-favorite-delete-invalid-listing-id-format" -Method "DELETE" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid/favorites" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "favorite-listing-delete-invalid-listing-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-consult-post-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid/consultations" -Body @{ channel = "FORM" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "consult-listing-post-invalid-listing-id-format") -Expected @(400))
  ...
  [void](Add-AdminConfigPutCaseResult -Results $results -Name "admin-config-home-landing-put" -Url "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing" -Body $adminHomeLandingConfig -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-config-home-landing-put") -Action "CONFIG_HOME_LANDING_UPDATE" -ApiPort $resolvedApiPort -AuthorizationToken $adminToken)
  [void](Add-ApiCaseResult -Results $results -Name "admin-config-home-landing-put-invalid-schema-version" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing" -Body @{ schemaVersion = 2 } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-config-home-landing-put-invalid-schema-version") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-issue-invoice-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/invoice" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-issue-invoice-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-upsert-invoice-invalid-order-id-format" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/invoice" -Body @{ invoiceFileId = [guid]::NewGuid().ToString() } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-upsert-invoice-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-delete-invoice-invalid-order-id-format" -Method "DELETE" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/invoice" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-delete-invoice-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-detail-invalid-order-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-manual-payment-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/payments/manual" -Body @{ payType = "DEPOSIT" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-manual-payment-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-contract-signed-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/milestones/contract-signed" -Body @{ dealAmountFen = 100 } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-contract-signed-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-transfer-completed-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/milestones/transfer-completed" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-transfer-completed-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-settlement-invalid-order-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/settlement" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-manual-payout-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/payouts/manual" -Body @{ payoutEvidenceFileId = [guid]::NewGuid().ToString() } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-manual-payout-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-get-invalid-order-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/invoice" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-request-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/invoice-requests" -Body $invoiceRequestBody -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-invoice-request-invalid-order-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-approve-invalid-verification-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/not-a-uuid/approve" -Body @{ comment = "smoke invalid verification id format approve" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-user-verification-approve-invalid-verification-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-reject-invalid-verification-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/not-a-uuid/reject" -Body @{ reason = "smoke invalid verification id format reject" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-user-verification-reject-invalid-verification-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-logo-update-missing" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/$missingVerificationId/logo" -Body @{ logoFileId = $null } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-user-verification-logo-update-missing") -Expected @(404))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-refund-approve-invalid-refund-request-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/refund-requests/not-a-uuid/approve" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-refund-approve-invalid-refund-request-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-refund-reject-invalid-refund-request-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/refund-requests/not-a-uuid/reject" -Body @{ reason = "smoke invalid refund request id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-refund-reject-invalid-refund-request-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-refund-complete-invalid-refund-request-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/refund-requests/not-a-uuid/complete" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-refund-complete-invalid-refund-request-id-format") -Expected @(400))

  $orderCreateHeaders = New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-create"
  ...
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-request-unauthorized" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice-requests" -Body $invoiceRequestBody -Headers @{} -Expected @(401))
  [void](Add-ApiCaseResult -Results $results -Name "order-detail-invalid-order-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-payment-intent-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/payment-intents" -Body @{ payType = "DEPOSIT" } -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-payment-intent-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-case-invalid-order-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/case" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-refund-requests-list-invalid-order-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/refund-requests" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-refund-request-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/not-a-uuid/refund-requests" -Body @{ reasonCode = "OTHER"; reasonText = "smoke invalid orderId format" } -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-refund-request-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-create-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders" -Body @{ listingId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-create-invalid-listing-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "order-payment-intent-invalid-pay-type" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/payment-intents" -Body @{ payType = "INVALID" } -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-payment-intent-invalid-pay-type") -Expected @(400))
  $orderPaymentIntentDepositHeaders = New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-payment-intent-deposit"
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-manual-payment-deposit-empty-trade-no" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/payments/manual" -Body @{ payType = "DEPOSIT"; tradeNo = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-manual-payment-deposit-empty-trade-no") -Expected @(400))
  ...
  $orderDetailAfterDeposit = Add-ApiCaseResult -Results $results -Name "order-detail-after-deposit-paid" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $evidenceUpload = Add-ApiFileUploadCaseResult -Results $results -Name "file-upload-evidence" -Url "http://127.0.0.1:$resolvedApiPort/files" -AuthorizationToken $orderSellerToken -FilePath $smokeEvidencePath -FormFields $null -Expected @(200, 201)
  ...
  $contractPdfUpload = Add-ApiFileUploadCaseResult -Results $results -Name "file-upload-contract-pdf" -Url "http://127.0.0.1:$resolvedApiPort/files" -AuthorizationToken $orderSellerToken -FilePath $smokeContractPdfPath -FormFields $null -Expected @(200, 201)
  ...
  $contractUpload = Add-ApiCaseResult -Results $results -Name "contract-upload" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = $contractPdfFileId } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload") -Expected @(200, 201)
  ...
  $contractListWaitConfirm = Add-ApiCaseResult -Results $results -Name "contract-list-wait-confirm" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?status=WAIT_CONFIRM" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $contractListWaitConfirmLowercase = Add-ApiCaseResult -Results $results -Name "contract-list-wait-confirm-lowercase" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?status=wait_confirm" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  Assert-ResultJsonArrayItemFieldEquals -Result $contractListWaitConfirmLowercase -ArrayField "items" -MatchField "orderId" -MatchValue $orderId -TargetField "status" -ExpectedValue "WAIT_CONFIRM" -Assertion "contract-list-wait-confirm-lowercase-status"
  $contractSignedSubmission = Add-ApiCaseResult -Results $results -Name "contract-signed-submission" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/contract-$orderId/signed-submissions" -Body @{ fileId = $contractPdfFileId } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-signed-submission") -Expected @(200, 201)
  $contractSignedSubmissionId = Get-ResultStringField -Result $contractSignedSubmission -Field "id"
  if ([string]::IsNullOrWhiteSpace($contractSignedSubmissionId)) { throw "contract-signed-submission missing id" }
  [void](Add-ApiCaseResult -Results $results -Name "order-refund-request-missing-reason-code" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/refund-requests" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-refund-request-missing-reason-code") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-refund-request-invalid-reason-code" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/refund-requests" -Body @{ reasonCode = "INVALID"; reasonText = "smoke invalid reasonCode" } -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-refund-request-invalid-reason-code") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-contract-signed-missing-submission" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/milestones/contract-signed" -Body @{ dealAmountFen = 2000000 } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-contract-signed-missing-submission") -Expected @(409))
  $adminOrderContractSigned = Add-ApiCaseResult -Results $results -Name "admin-order-contract-signed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/milestones/contract-signed" -Body @{ dealAmountFen = 2000000; signedSubmissionId = $contractSignedSubmissionId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-contract-signed") -Expected @(200, 201)
  ...
  $orderDetailAfterContractSigned = Add-ApiCaseResult -Results $results -Name "order-detail-after-contract-signed" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "contract-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-wait-upload" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?status=WAIT_UPLOAD" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-empty-status" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?status=" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-status" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?status=UNKNOWN" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-negative" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=-1" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-zero" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=0" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-size-negative" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=-1" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-decimal" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=1.5" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-size-zero" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=0" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-size-decimal" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=1.5" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page-size" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=abc" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-invalid-page" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=abc" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-page-one" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=1" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200))
  $contractListPageSizeOne = Add-ApiCaseResult -Results $results -Name "contract-list-page-size-one" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=1" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-empty-page" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-list-empty-page-size" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(400))
  $contractListPageOnePageSizeFifty = Add-ApiCaseResult -Results $results -Name "contract-list-page-one-page-size-fifty" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?page=1&pageSize=50" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  Assert-ResultJsonFieldEquals -Result $contractListPageOnePageSizeFifty -Field "page.pageSize" -ExpectedValue 50 -Assertion "contract-list-page-one-page-size-fifty-reflects"
  $contractListPageSizeFifty = Add-ApiCaseResult -Results $results -Name "contract-list-page-size-fifty" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=50" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $contractListPageSizeCapped = Add-ApiCaseResult -Results $results -Name "contract-list-page-size-capped" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=999" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $contractListPageSizeSlightOverCap = Add-ApiCaseResult -Results $results -Name "contract-list-page-size-slight-over-cap" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/contracts?pageSize=51" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $orderPaymentIntentFinalHeaders = New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-payment-intent-final"
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-manual-payment-final-empty-trade-no" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/payments/manual" -Body @{ payType = "FINAL"; tradeNo = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-manual-payment-final-empty-trade-no") -Expected @(400))
  ...
  $orderDetailAfterFinalPaid = Add-ApiCaseResult -Results $results -Name "order-detail-after-final-paid" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $orderCaseAfterTransfer = Add-ApiCaseResult -Results $results -Name "order-case-after-transfer-completed" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/case" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  $orderDetailAfterTransfer = Add-ApiCaseResult -Results $results -Name "order-detail-after-transfer-completed" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId" -Body $null -Headers @{ Authorization = $orderBuyerToken } -Expected @(200)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-upsert-invoice-invalid-file-id-format" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/invoice" -Body @{ invoiceFileId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-upsert-invoice-invalid-file-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "order-refund-request-not-allowed-ready-to-settle" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/refund-requests" -Body @{ reasonCode = "OTHER"; reasonText = "smoke not allowed" } -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-refund-request-not-allowed-ready-to-settle") -Expected @(409))
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-request-not-completed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice-requests" -Body $invoiceRequestBody -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-invoice-request-not-completed") -Expected @(409))
  [void](Add-ApiCaseResult -Results $results -Name "file-download-invalid-file-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/files/not-a-uuid" -Body $null -Headers @{ Authorization = $orderSellerToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "file-preview-invalid-file-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/files/not-a-uuid/preview" -Body $null -Headers @{ Authorization = $orderSellerToken } -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "file-download" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/files/$evidenceFileId" -Body $null -Headers @{ Authorization = $orderSellerToken } -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "file-preview" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/files/$evidenceFileId/preview" -Body $null -Headers @{ Authorization = $orderSellerToken } -Expected @(200))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-missing-contract-id" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$missingContractUploadId/upload" -Body @{ contractFileId = $evidenceFileId } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-missing-contract-id") -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-forbidden-not-seller" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = $evidenceFileId } -Headers (New-WriteHeaders -AuthorizationToken $orderNonSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-forbidden-not-seller") -Expected @(403))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-invalid-contract-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/not-a-uuid/upload" -Body @{ contractFileId = [guid]::NewGuid().ToString() } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-invalid-contract-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-invalid-prefixed-contract-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/contract-not-a-uuid/upload" -Body @{ contractFileId = [guid]::NewGuid().ToString() } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-invalid-prefixed-contract-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-invalid-contract-file-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-invalid-contract-file-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-empty-contract-file-id" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = "" } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-empty-contract-file-id") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-missing-contract-file-id" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-missing-contract-file-id") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-contract-file-id-not-found" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = [guid]::NewGuid().ToString() } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-contract-file-id-not-found") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-non-pdf-contract-file" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = $evidenceFileId } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-non-pdf-contract-file") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "contract-upload-after-confirmation" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/contracts/$orderId/upload" -Body @{ contractFileId = $contractPdfFileId } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "contract-upload-after-confirmation") -Expected @(409))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "file-temporary-access-invalid-file-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/files/not-a-uuid/temporary-access" -Body @{ scope = "preview"; ttlSeconds = 600 } -Headers (New-WriteHeaders -AuthorizationToken $orderSellerToken -Prefix $idempotencyPrefix -Label "file-temporary-access-invalid-file-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-manual-payout-invalid-payout-evidence-file-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/payouts/manual" -Body @{ payoutEvidenceFileId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-manual-payout-invalid-payout-evidence-file-id-format") -Expected @(400))
  ...
  $orderInvoiceRequestHeaders = New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-invoice-request-completed"
  $orderInvoiceRequestCompleted = Add-ApiCaseResult -Results $results -Name "order-invoice-request-completed" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice-requests" -Body $invoiceRequestBody -Headers $orderInvoiceRequestHeaders -Expected @(200, 201)
  ...
  $orderInvoiceRequestCompletedReplay = Add-ApiCaseResult -Results $results -Name "order-invoice-request-completed-idempotent-replay" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice-requests" -Body $invoiceRequestBody -Headers $orderInvoiceRequestHeaders -Expected @(200, 201)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "order-invoice-request-completed-duplicate" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice-requests" -Body $invoiceRequestBody -Headers (New-WriteHeaders -AuthorizationToken $orderBuyerToken -Prefix $idempotencyPrefix -Label "order-invoice-request-completed-duplicate") -Expected @(409))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-upsert-invoice-empty-invoice-no" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/invoice" -Body @{ invoiceFileId = $evidenceFileId; invoiceNo = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-upsert-invoice-empty-invoice-no") -Expected @(400))
  ...
  $orderInvoiceGetAfterDelete = Add-ApiCaseResult -Results $results -Name "order-invoice-get-after-delete" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/orders/$orderId/invoice" -Body $null -Headers @{ Authorization = $userToken } -Expected @(200)
  Assert-ResultJsonFieldEquals -Result $orderInvoiceGetAfterDelete -Field "status" -ExpectedValue "APPLYING" -Assertion "invoice-get-status-after-delete"
  ...
  if ($mixedRaceInvoiceResult -and [int]$mixedRaceInvoiceResult.status -eq 409) {
    $mixedRaceInvoiceRequestAfter = Add-ApiCaseResult -Results $results -Name "mixed-race-order-invoice-request-after-payout" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/orders/$mixedRaceOrderId/invoice-requests" -Body $invoiceRequestBody -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "mixed-race-order-invoice-request-after-payout") -Expected @(200, 201)
  ...
  foreach ($multiAggSeedBatch in $multiAggSeedBatches) {
    ...
    foreach ($multiAggSeed in $multiAggSeedBatch) {
      ...
      $multiAggInvoicePairResults = Add-ConcurrentApiCasePairResults -Results $results -NameA $multiAggInvoiceNameA -MethodA "POST" -UrlA "http://127.0.0.1:$resolvedApiPort/orders/$multiAggOrderAId/invoice-requests" -BodyA $invoiceRequestBody -HeadersA (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label $multiAggInvoiceNameA) -NameB $multiAggInvoiceNameB -MethodB "POST" -UrlB "http://127.0.0.1:$resolvedApiPort/orders/$multiAggOrderBId/invoice-requests" -BodyB $invoiceRequestBody -HeadersB (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label $multiAggInvoiceNameB) -Expected @(200, 201, 409)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-create-empty-title" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases" -Body @{ type = "DISPUTE"; title = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-create-empty-title") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-create-empty-requester-name" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases" -Body @{ type = "DISPUTE"; title = "smoke case empty requesterName $ReportDate"; requesterName = "" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-create-empty-requester-name") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-create-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases" -Body @{ type = "DISPUTE"; title = "smoke case invalid orderId format $ReportDate"; orderId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-create-invalid-order-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-create-invalid-assignee-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases" -Body @{ type = "DISPUTE"; title = "smoke case invalid assigneeId format $ReportDate"; assigneeId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-create-invalid-assignee-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-detail-invalid-id" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-assign-invalid-case-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/not-a-uuid/assign" -Body @{ assigneeId = $currentUserId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-assign-invalid-case-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-status-invalid-case-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/not-a-uuid/status" -Body @{ status = "IN_PROGRESS" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-status-invalid-case-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-note-invalid-case-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/not-a-uuid/notes" -Body @{ note = "smoke invalid case id format note" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-note-invalid-case-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-evidence-invalid-case-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/not-a-uuid/evidence" -Body @{ fileId = $evidenceFileId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-evidence-invalid-case-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-sla-invalid-case-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/not-a-uuid/sla" -Body @{ dueAt = (Get-Date).AddDays(2).ToString("o") } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-sla-invalid-case-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-assign-invalid-assignee-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/$caseId/assign" -Body @{ assigneeId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-assign-invalid-assignee-format") -Expected @(400))
  $caseAssign = Add-ApiCaseResult -Results $results -Name "admin-case-assign" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/$caseId/assign" -Body @{ assigneeId = $adminUserId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-assign") -Expected @(200, 201)
  Assert-ResultJsonFieldEquals -Result $caseAssign -Field "assigneeId" -ExpectedValue $adminUserId -Assertion "case-assignee-linked"
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-note-empty" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/$caseId/notes" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-note-empty") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-evidence-invalid-file-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/$caseId/evidence" -Body @{ fileId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-evidence-invalid-file-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-case-evidence-invalid-file-id" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/cases/$caseId/evidence" -Body @{ fileId = [guid]::NewGuid().ToString() } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-case-evidence-invalid-file-id") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-maintenance-schedule-detail-invalid-schedule-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-maintenance/schedules/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-maintenance-schedule-update-invalid-schedule-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-maintenance/schedules/not-a-uuid" -Body @{ status = "PAID" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-maintenance-schedule-update-invalid-schedule-id-format") -Expected @(400))
  ...
  $maintenanceAssigneeUserId = $adminUserId
  $taskCreate = Add-ApiCaseResult -Results $results -Name "admin-maintenance-task-create" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-maintenance/tasks" -Body @{ scheduleId = $scheduleId; assignedCsUserId = $maintenanceAssigneeUserId; status = "OPEN"; note = "smoke maintenance task" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-maintenance-task-create") -Expected @(200, 201)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-maintenance-task-update-invalid-task-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-maintenance/tasks/not-a-uuid" -Body @{ status = "DONE" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-maintenance-task-update-invalid-task-id-format") -Expected @(400))
  ...
  $maintenanceOrderCreate = Add-ApiCaseResult -Results $results -Name "admin-maintenance-order-create" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-maintenance/orders" -Body @{ scheduleId = $maintenanceOrderScheduleId; applicantUserId = $currentUserId; assignedCsUserId = $maintenanceAssigneeUserId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-maintenance-order-create") -Expected @(200, 201)
  ...
  $maintenanceCancelOrderCreate = Add-ApiCaseResult -Results $results -Name "admin-maintenance-order-create-for-cancel" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-maintenance/orders" -Body @{ scheduleId = $maintenanceCancelScheduleId; applicantUserId = $currentUserId; assignedCsUserId = $maintenanceAssigneeUserId } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-maintenance-order-create-for-cancel") -Expected @(200, 201)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-rbac-role-update-invalid-role-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/rbac/roles/not-a-uuid" -Body @{ name = "invalid role id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-rbac-role-update-invalid-role-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-rbac-user-update-invalid-user-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/rbac/users/not-a-uuid" -Body @{ roleIds = @($rbacRoleId) } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-rbac-user-update-invalid-user-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-logo-update-unauthorized" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/$missingVerificationId/logo" -Body @{ logoFileId = $null } -Headers @{} -Expected @(401))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-config-home-landing-put-unauthorized" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing" -Body $adminHomeLandingConfig -Headers @{} -Expected @(401))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-config-home-landing-put-custom-rbac-role-forbidden" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing" -Body $adminHomeLandingConfig -Headers @{ Authorization = $userToken } -Expected @(403))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-logo-update-custom-rbac-role-forbidden" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/$missingVerificationId/logo" -Body @{ logoFileId = $null } -Headers @{ Authorization = $userToken } -Expected @(403))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-config-home-landing-put-after-clear-roles-forbidden" -Method "PUT" -Url "http://127.0.0.1:$resolvedApiPort/admin/config/home-landing" -Body $adminHomeLandingConfig -Headers @{ Authorization = $userToken } -Expected @(403))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-user-verification-logo-update-after-clear-roles-forbidden" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/$missingVerificationId/logo" -Body @{ logoFileId = $null } -Headers @{ Authorization = $userToken } -Expected @(403))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-rbac-role-delete-invalid-role-id-format" -Method "DELETE" -Url "http://127.0.0.1:$resolvedApiPort/admin/rbac/roles/not-a-uuid" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-rbac-role-delete-invalid-role-id-format") -Expected @(400))
  ...
  # The transaction case completes and off-shelves $listingId; comments and consultations require an active listing.
  $commentListingId = New-OrderableListingForScenario -Results $results -ApiPort $resolvedApiPort -UserToken $userToken -AdminToken $adminToken -IdempotencyPrefix $idempotencyPrefix -CasePrefix "listing-engagement"
  [void](Add-ApiCaseResult -Results $results -Name "listing-comment-create-unauthorized" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/comments" -Body @{ text = "smoke unauthorized listing comment" } -Headers @{} -Expected @(401))
  $listingCommentCreate = Add-ApiCaseResult -Results $results -Name "listing-comment-create" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/comments" -Body @{ text = "smoke listing comment $ReportDate" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "comment-listing-create") -Expected @(200, 201)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-comment-create-empty-text" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/comments" -Body @{ text = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "comment-listing-empty") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "listing-comment-create-empty-parent-comment-id" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/comments" -Body @{ text = "smoke listing comment empty parent"; parentCommentId = "" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "comment-listing-empty-parent") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-comment-create-invalid-parent-comment-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/comments" -Body @{ text = "smoke invalid parent comment id format"; parentCommentId = "not-a-uuid" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "comment-listing-invalid-parent-id-format") -Expected @(400))
  ...
  $adminCommentsHiddenList = Add-ApiCaseResult -Results $results -Name "admin-comments-hidden-filter-after-update" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/comments?contentId=$commentListingId&status=HIDDEN" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200)
  ...
  $adminCommentsVisibleList = Add-ApiCaseResult -Results $results -Name "admin-comments-visible-filter-after-restore" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/comments?contentId=$commentListingId&status=VISIBLE" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "me-address-update-invalid-address-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/me/addresses/not-a-uuid" -Body @{ addressLine = "Smoke Street Invalid Address Id"; isDefault = $false } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "address-update-invalid-address-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "me-address-delete-invalid-address-id-format" -Method "DELETE" -Url "http://127.0.0.1:$resolvedApiPort/me/addresses/not-a-uuid" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "address-delete-invalid-address-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "listing-conversation-upsert-unauthorized" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/conversations" -Body $null -Headers @{} -Expected @(401))
  [void](Add-ApiCaseResult -Results $results -Name "listing-conversation-upsert-invalid-listing-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/not-a-uuid/conversations" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "listing-conversation-upsert-invalid-listing-id-format") -Expected @(400))
  $listingConversation = Add-ApiCaseResult -Results $results -Name "listing-conversation-upsert" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/listings/$commentListingId/conversations" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "conversation-listing-upsert") -Expected @(200, 201)
  ...
  [void](Add-ApiCaseResult -Results $results -Name "tech-manager-conversation-upsert-invalid-tech-manager-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/tech-managers/not-a-uuid/conversations" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "tech-manager-conversation-upsert-invalid-tech-manager-id-format") -Expected @(400))
  ...
  # Coverage probes for first-party operations that are otherwise not reached by the main business flows.
  $missingFileId = [guid]::NewGuid().ToString()
  $missingImportBatchId = [guid]::NewGuid().ToString()
  [void](Add-ApiCaseResult -Results $results -Name "admin-file-moderation-forbidden" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/files/$missingFileId/moderation" -Body @{ status = "APPROVED" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "admin-file-moderation-forbidden") -Expected @(403))
  [void](Add-ApiCaseResult -Results $results -Name "admin-file-moderation-missing-file" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/files/$missingFileId/moderation" -Body @{ status = "APPROVED" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-file-moderation-missing-file") -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "public-home-stats" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/public/home-stats" -Body $null -Headers @{} -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "my-conversation-detail" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/me/conversations/$listingConversationId" -Body $null -Headers @{ Authorization = $userToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "my-maintenance-listing-request-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/me/patent-maintenance/listing-requests" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-maintenance-listing-request-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-maintenance-direct-request-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/me/patent-maintenance/direct-requests" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-maintenance-direct-request-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-maintenance-summary" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/me/patent-maintenance/summary" -Body $null -Headers @{ Authorization = $userToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-tech-manager-badges-invalid-body" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/tech-managers/batch/badges" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-tech-manager-badges-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-verification-profile-invalid-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/user-verifications/not-a-uuid/profile" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-verification-profile-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-notification-badges" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/notifications/badges" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-refund-requests-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/refund-requests" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-settlements-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/settlements" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-invoices-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/invoices" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-signed-submission-reject-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/not-a-uuid/contract/signed-submissions/not-a-uuid/reject" -Body @{ reason = "smoke invalid order id" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-order-signed-submission-reject-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-order-invoice-detail" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders/$orderId/invoice" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-orders-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/orders" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-assigned-orders-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/assigned-orders" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-assigned-order-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/assigned-orders/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-assigned-contract-signed-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/assigned-orders/not-a-uuid/milestones/contract-signed" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-assigned-contract-signed-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-assigned-signed-submission-reject-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/assigned-orders/not-a-uuid/contract/signed-submissions/not-a-uuid/reject" -Body @{ reason = "smoke invalid order id" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-assigned-signed-submission-reject-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-assigned-contract-upload-invalid-order-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/assigned-orders/not-a-uuid/contract/upload" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-assigned-contract-upload-invalid-order-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-dashboard-showcase-summary" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/dashboard/showcase-summary" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-ops-notification-jobs-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/ops-notification-jobs" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-ops-notification-job-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/ops-notification-jobs/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-records-summary" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/summary" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-imports-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/imports" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-import-rows-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/imports/not-a-uuid/rows" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-import-preview-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/import/preview" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-deal-record-import-preview-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-import-execute-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/import/execute" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-deal-record-import-execute-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batches-forbidden" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches" -Body $null -Headers @{ Authorization = $userToken } -Expected @(403))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batches-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batch-detail-missing" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches/$missingImportBatchId" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batch-changes-missing" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches/$missingImportBatchId/changes" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batch-rollback-preview-missing" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches/$missingImportBatchId/rollback-preview" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-import-batch-rollback-preview-missing") -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batch-rollback-missing" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches/$missingImportBatchId/rollback" -Body @{ reason = "smoke missing import batch" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-import-batch-rollback-missing") -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-import-batch-rollback-report-missing" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/import-batches/$missingImportBatchId/rollback-report" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-backfill-invalid-limit" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/backfill/completed-orders" -Body @{ limit = 0 } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-deal-record-backfill-invalid-limit") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-records-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-deal-record-void-invalid-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/deal-records/not-a-uuid/void" -Body @{ reason = "smoke invalid record id" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-deal-record-void-invalid-id-format") -Expected @(400))

  # Coverage probes for newly added operations (achievement/admin batch/claims/platform conversation).
  [void](Add-ApiCaseResult -Results $results -Name "me-favorites-achievements-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/me/favorites/achievements" -Body $null -Headers @{ Authorization = $userToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "public-achievement-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/public/achievements/not-a-uuid" -Body $null -Headers @{} -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "public-achievement-comments-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/public/achievements/not-a-uuid/comments" -Body $null -Headers @{} -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "search-achievements" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/search/achievements" -Body $null -Headers @{} -Expected @(200))

  [void](Add-ApiCaseResult -Results $results -Name "my-achievements-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/achievements" -Body $null -Headers @{ Authorization = $userToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-create-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/achievements" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-create-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid" -Body $null -Headers @{ Authorization = $userToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-update-invalid-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid" -Body @{ title = "invalid id format" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-update-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-submit-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid/submit" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-submit-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-off-shelf-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid/off-shelf" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-off-shelf-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-consultation-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid/consultations" -Body @{ channel = "FORM" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-consultation-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-comment-create-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid/comments" -Body @{ text = "invalid achievement id format" } -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-comment-create-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-favorite-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid/favorites" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-favorite-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "my-achievement-unfavorite-invalid-id-format" -Method "DELETE" -Url "http://127.0.0.1:$resolvedApiPort/achievements/not-a-uuid/favorites" -Body $null -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "my-achievement-unfavorite-invalid-id-format") -Expected @(400))
  ...
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievements-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-create-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-create-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-update-invalid-id-format" -Method "PATCH" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid" -Body @{ title = "invalid id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-update-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-publish-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid/publish" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-publish-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-off-shelf-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid/off-shelf" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-off-shelf-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-materials-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid/materials" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-audit-logs-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid/audit-logs" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-approve-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid/approve" -Body @{ reason = "invalid id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-approve-invalid-id-format") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-achievement-reject-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/achievements/not-a-uuid/reject" -Body @{ reason = "invalid id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-achievement-reject-invalid-id-format") -Expected @(400))

  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-jobs-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-job-create-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-import-job-create-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-job-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-job-validate-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import/not-a-uuid/validate" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-import-job-validate-invalid-id-format") -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-job-execute-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import/not-a-uuid/execute" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-import-job-execute-invalid-id-format") -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-job-rows-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import/not-a-uuid/rows" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-import-job-error-file-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/import/not-a-uuid/error-file" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-generate-listings-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patents/jobs/listings" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-generate-listings-invalid-body") -Expected @(400))

  [void](Add-ApiCaseResult -Results $results -Name "me-patent-claims-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/me/patent-claims" -Body $null -Headers @{ Authorization = $userToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "me-patent-claim-create-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/me/patent-claims" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $userToken -Prefix $idempotencyPrefix -Label "me-patent-claim-create-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-claims-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-claims" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-claim-approve-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-claims/not-a-uuid/approve" -Body @{ reason = "invalid id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-claim-approve-invalid-id-format") -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-patent-claim-reject-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/patent-claims/not-a-uuid/reject" -Body @{ reason = "invalid id format" } -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-patent-claim-reject-invalid-id-format") -Expected @(400, 404))

  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-batch-jobs-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/batch" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-batch-job-create-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/batch" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-batch-job-create-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-batch-job-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/batch/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-batch-job-items-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/batch/not-a-uuid/items" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-batch-job-error-file-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/batch/not-a-uuid/error-file" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))

  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-jobs-list" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-job-create-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-import-job-create-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-job-detail-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import/not-a-uuid" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-job-validate-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import/not-a-uuid/validate" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-import-job-validate-invalid-id-format") -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-job-execute-invalid-id-format" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import/not-a-uuid/execute" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-listing-import-job-execute-invalid-id-format") -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-job-rows-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import/not-a-uuid/rows" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-listing-import-job-error-file-invalid-id-format" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/listings/jobs/import/not-a-uuid/error-file" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400, 404))
  [void](Add-ApiCaseResult -Results $results -Name "admin-people-achievements-import-preview-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/imports/people-achievements/preview" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-people-achievements-import-preview-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-people-achievements-import-execute-invalid-body" -Method "POST" -Url "http://127.0.0.1:$resolvedApiPort/admin/imports/people-achievements/execute" -Body @{} -Headers (New-WriteHeaders -AuthorizationToken $adminToken -Prefix $idempotencyPrefix -Label "admin-people-achievements-import-execute-invalid-body") -Expected @(400))
  [void](Add-ApiCaseResult -Results $results -Name "admin-people-achievements-import-history" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/imports/people-achievements/history" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(200))
  [void](Add-ApiCaseResult -Results $results -Name "admin-people-achievements-import-history-invalid-action" -Method "GET" -Url "http://127.0.0.1:$resolvedApiPort/admin/imports/people-achievements/history?action=INVALID" -Body $null -Headers @{ Authorization = $adminToken } -Expected @(400))
