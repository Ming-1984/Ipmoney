...
type SmsSendResponse = { cooldownSeconds: number };
...
export function LoginPage() {
  ...
  const validateSessionAndEnter = async (token: string) => {
    ...
    try {
      ...
      if (!session?.isAdmin) {
        throw new Error('当前账号不具备后台访问权限，请联系管理员开通。');
      }
  ...
  const onTokenLogin = async (values: { token: string }) => {
    ...
    try {
      ...
      const msg = e?.message || '登录失败，请检查访问令牌和权限配置。';
      setAuthError(msg);
      message.error(msg);
    } finally {
      setSubmitting(false);
  ...
  const onSmsLogin = async (values: { phone: string; code: string }) => {
    ...
    try {
      ...
      if (!token) throw new Error('登录返回的访问令牌为空');
      await validateSessionAndEnter(token);
    } catch (e: any) {
    ...
    >
      <Card style={{ width: 460 }}>
        ...
        <Typography.Paragraph type="secondary">
          推荐使用手机号验证码登录。访问令牌登录可用于已授权的技术运维场景。
        ...
        <Tabs
          ...
          items={[
            ...
            },
            {
              ...
              label: '访问令牌登录',
              children: (
                <Form form={tokenForm} layout="vertical" onFinish={onTokenLogin}>
                  <Form.Item
                    label="Access Token"
                    name="token"
                    rules={[
                      { required: true, message: '请输入访问令牌' },
                      { min: 8, message: '访问令牌长度至少为 8 位' },
                    ]}
                  >
