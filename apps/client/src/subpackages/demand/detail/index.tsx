...
export default function DemandDetailPage() {
  ...
  useShareAppMessage(() => ({
    title: data?.title ? `产学研需求：${data.title}` : '需求详情',
    path: demandId ? `/subpackages/demand/detail/index?demandId=${demandId}` : '/pages/home/index',
  ...
  const openPublisher = useCallback(() => {
    const orgUserId = data?.publisher?.userId;
    if (!orgUserId) {
      toast('暂无机构主页');
      return;
    }
    Taro.navigateTo({ url: `/subpackages/organizations/detail/index?orgUserId=${orgUserId}` });
  ...
  const mediaList = useMemo(() => {
    const list = media.filter((item) => item.url || item.fileId);
            ...
            <Surface className="detail-section-card patent-provider-card">
              {data.publisher ? (
                <View className="patent-provider-row">
                  ...
                  <Button block={false} size="small" variant="ghost" onClick={openPublisher}>
                    主页
                  </Button>
                </View>
              ) : (
