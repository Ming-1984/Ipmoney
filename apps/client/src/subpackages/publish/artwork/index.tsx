  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (!artworkId) {
        ...
        if (!req) return;
        res = await apiPost<Artwork>('/artworks', req, { idempotencyKey: `artwork-create-${Date.now()}` });
        setArtworkId(res.id);
      } else {
        const req = buildUpdate('save');
        if (!req) return;
        res = await apiPatch<Artwork>(`/artworks/${artworkId}`, req, { idempotencyKey: `artwork-patch-${artworkId}` });
      }
  ...
  const submitForAudit = useCallback(async () => {
    ...
    try {
      ...
      if (!id) {
        ...
        if (!req) return;
        const created = await apiPost<Artwork>('/artworks', req, { idempotencyKey: `artwork-create-submit-${Date.now()}` });
        id = created.id;
        ...
        if (!req) return;
        const updated = await apiPatch<Artwork>(`/artworks/${id}`, req, { idempotencyKey: `artwork-patch-submit-${id}` });
        setAuditStatus(updated.auditStatus || null);
        setStatus(updated.status || null);
      }

      const res = await apiPost<Artwork>(`/artworks/${id}/submit`, undefined, { idempotencyKey: `artwork-submit-${id}` });
      setAuditStatus(res.auditStatus || null);
      setStatus(res.status || null);
  ...
  const offShelf = useCallback(async () => {
    ...
    try {
      const res = await apiPost<Artwork>(
        ...
        { reason: '发布方下架' },
        { idempotencyKey: `artwork-off-${artworkId}` },
      );
      setAuditStatus(res.auditStatus || null);
      setStatus(res.status || null);
