...
type DemandDraftData = {
  ...
  budgetMinYuan: string;
  budgetMaxYuan: string;
...
const DRAFT_KEY = STORAGE_KEYS.publishDemandDraft;
...
function readDraft(): DemandDraft | null {
  try {
    ...
    return parsed as DemandDraft;
  } catch {
    return null;
  }
}

function hasDraftContent(data: DemandDraftData): boolean {
  if (!data) return false;
  if (data.title.trim() || data.summary.trim() || data.description.trim()) return true;
  if (data.keywords.length || data.cooperationModes.length || data.industryTags.length) return true;
  if (data.deliveryPeriod || data.budgetType === 'FIXED') return true;
  if (data.budgetMinYuan.trim() || data.budgetMaxYuan.trim()) return true;
  if (data.contactName.trim() || data.contactTitle.trim() || data.contactPhoneMasked.trim()) return true;
  if (data.regionCode.trim()) return true;
  if (data.coverFileId || data.coverUrl) return true;
  if (data.media.length) return true;
  return false;
}

function writeDraft(data: DemandDraftData) {
  if (!hasDraftContent(data)) {
    Taro.removeStorageSync(DRAFT_KEY);
    return;
  }
  const draft: DemandDraft = { version: DRAFT_VERSION, updatedAt: new Date().toISOString(), data };
...
export default function PublishDemandPage() {
  ...
  useEffect(() => {
    if (initialDemandId || demandId) return;
    ...
    setDeliveryPeriod((d.deliveryPeriod || '') as DeliveryPeriod | '');
    setBudgetType((d.budgetType || 'NEGOTIABLE') as PriceType | '');
    setBudgetMinYuan(d.budgetMinYuan || '');
    setBudgetMaxYuan(d.budgetMaxYuan || '');
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (!demandId) {
        ...
        if (!req) return;
        res = await apiPost<Demand>('/demands', req, { idempotencyKey: `demand-create-${Date.now()}` });
        setDemandId(res.id);
      } else {
        const req = buildUpdate('save');
        if (!req) return;
        res = await apiPatch<Demand>(`/demands/${demandId}`, req, { idempotencyKey: `demand-patch-${demandId}` });
      }
  ...
  const submitForAudit = useCallback(async () => {
    ...
    try {
      ...
      if (!id) {
        ...
        if (!req) return;
        const created = await apiPost<Demand>('/demands', req, { idempotencyKey: `demand-create-submit-${Date.now()}` });
        id = created.id;
        ...
        if (!req) return;
        const updated = await apiPatch<Demand>(`/demands/${id}`, req, { idempotencyKey: `demand-patch-submit-${id}` });
        setAuditStatus(updated.auditStatus || null);
        setStatus(updated.status || null);
      }

      const res = await apiPost<Demand>(`/demands/${id}/submit`, undefined, { idempotencyKey: `demand-submit-${id}` });
      setAuditStatus(res.auditStatus || null);
      setStatus(res.status || null);
      clearDraft();
      const goList = await confirm({
        title: '提交成功',
        content: '已进入审核中，可在「我的需求」查看进度。',
        confirmText: '去我的需求',
        cancelText: '留在此页',
      });
      if (goList) {
  ...
  const offShelf = useCallback(async () => {
    ...
    setOffShelving(true);
    try {
      const res = await apiPost<Demand>(`/demands/${demandId}/off-shelf`, { reason: '发布方下架' }, { idempotencyKey: `demand-off-${demandId}` });
      setAuditStatus(res.auditStatus || null);
      setStatus(res.status || null);
