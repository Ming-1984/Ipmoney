...
const DRAFT_KEY = STORAGE_KEYS.publishAchievementDraft;
...
function readDraft(): AchievementDraft | null {
  try {
    ...
    return parsed as AchievementDraft;
  } catch {
    return null;
  }
}

function hasDraftContent(data: AchievementDraftData): boolean {
...
function writeDraft(data: AchievementDraftData) {
  if (!hasDraftContent(data)) {
    Taro.removeStorageSync(DRAFT_KEY);
    return;
  }
  const draft: AchievementDraft = { version: DRAFT_VERSION, updatedAt: new Date().toISOString(), data };
...
export default function PublishAchievementPage() {
  ...
  useEffect(() => {
    if (initialAchievementId || achievementId) return;
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      const data: AchievementDraftData = {
        title,
        summary,
        description,
        keywords,
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (!achievementId) {
        ...
        if (!req) return;
        res = await apiPost<Achievement>('/achievements', req, { idempotencyKey: `achievement-create-${Date.now()}` });
        setAchievementId(res.id);
      } else {
        const req = buildUpdate('save');
        if (!req) return;
        res = await apiPatch<Achievement>(`/achievements/${achievementId}`, req, { idempotencyKey: `achievement-patch-${achievementId}` });
      }
  ...
  const submitForAudit = useCallback(async () => {
    ...
    try {
      ...
      if (!id) {
        ...
        if (!req) return;
        const created = await apiPost<Achievement>('/achievements', req, { idempotencyKey: `achievement-create-submit-${Date.now()}` });
        id = created.id;
        ...
        if (!req) return;
        const updated = await apiPatch<Achievement>(`/achievements/${id}`, req, { idempotencyKey: `achievement-patch-submit-${id}` });
        setAuditStatus(updated.auditStatus || null);
        setStatus(updated.status || null);
      }

      const res = await apiPost<Achievement>(`/achievements/${id}/submit`, undefined, { idempotencyKey: `achievement-submit-${id}` });
      setAuditStatus(res.auditStatus || null);
      setStatus(res.status || null);
      clearDraft();
      const goList = await confirm({
        title: '提交成功',
        content: '已进入审核中，可在「我的成果」查看进度。',
        confirmText: '去我的成果',
        cancelText: '留在此页',
      });
      if (goList) {
  ...
  const offShelf = useCallback(async () => {
    ...
    setOffShelving(true);
    try {
      const res = await apiPost<Achievement>(`/achievements/${achievementId}/off-shelf`, { reason: '发布方下架' }, { idempotencyKey: `achievement-off-${achievementId}` });
      setAuditStatus(res.auditStatus || null);
      setStatus(res.status || null);
